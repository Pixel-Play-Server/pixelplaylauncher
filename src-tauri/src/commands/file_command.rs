...
use crate::integrations::pixelplay_packs::pixelplayModEntryDefinition;
...
/// Fetches the first PNG icon found within pixelplay Pack mods as Base64 strings.
///
/// # Arguments
///
/// * `mods` - A vector of pixelplayModEntryDefinition structs
...
#[tauri::command]
pub async fn get_icons_for_pixelplay_mods(
    mods: Vec<pixelplayModEntryDefinition>,
    minecraft_version: String,
    loader: String,
) -> Result<HashMap<String, Option<String>>, CommandError> {
    info!("Fetching icons for {} pixelplay Pack mods...", mods.len());
    let mut results_map: HashMap<String, Option<String>> = HashMap::new();
    ...
    for mod_entry in &mods {
        match path_utils::get_pixelplay_mod_cache_path(mod_entry, &minecraft_version, &loader) {
            Ok(path) => {
    ...
    info!(
        "Finished fetching pixelplay mod icons. Returning {} results.",
        results_map.len()
    );
    Ok(results_map)
}
