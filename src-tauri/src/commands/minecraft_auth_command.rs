...
/// Create an offline account with the provided username and set it active.
#[tauri::command]
pub async fn create_offline_account(username: String) -> Result<Credentials, CommandError> {
    if username.trim().is_empty() {
        return Err(CommandError::from(AppError::AccountError("Username cannot be empty".to_string())));
    }
...
/// Create an offline account with a provided UUID string and username
#[tauri::command]
pub async fn create_offline_account_with_id(id: String, username: String) -> Result<Credentials, CommandError> {
    if username.trim().is_empty() {
        return Err(CommandError::from(AppError::AccountError("Username cannot be empty".to_string())));
    }

    let parsed = Uuid::parse_str(&id).map_err(|e| CommandError::from(AppError::Other(e.to_string())))?;
    let state = State::get().await?;
    let creds = state
        .minecraft_account_manager_v2
        .create_offline_account_with_id(parsed, &username)
        .await?;
    Ok(creds)
}
