...
pub async fn browse_capes(
    ...
    // Get the pixelplay token: prioritize passed token, otherwise get from active account
    let token_to_use = match payload.pixelplay_token {
        Some(token) => {
            debug!("Using provided pixelplay token.");
            token
        }
        None => {
...
pub struct GetPlayerCapesPayload {
    ...
    pub pixelplay_token: Option<String>,
...
/// - pixelplay_token: Optional pixelplay token
#[tauri::command]
pub async fn get_player_capes(
    ...
    let token_to_use = match payload.pixelplay_token {
        Some(token) => {
            debug!("[CMD get_player_capes] Using pixelplay_token from payload.");
            token
        }
        None => {
            debug!("[CMD get_player_capes] No pixelplay_token in payload, attempting to use token from active account.");
            let acc = active_account_opt.as_ref().ok_or_else(|| {
                error!("[CMD get_player_capes] pixelplay token required (neither in payload nor from active account).");
                CommandError::from(AppError::NoCredentialsError)
            })?;
            acc.pixelplay_credentials.get_token_for_mode(is_experimental)?
        }
    };
    debug!(
...
pub async fn equip_cape(
    ...
    // Get the pixelplay token: prioritize passed token, otherwise get from active account
    let token_to_use = match pixelplay_token {
        Some(token) => {
            debug!("Using provided pixelplay token.");
            token
        }
