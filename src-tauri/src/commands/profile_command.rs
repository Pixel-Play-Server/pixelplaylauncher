...
pub struct CreateProfileParams {
    ...
    selected_pixelplay_pack_id: Option<String>,
...
pub struct UpdateProfileParams {
    ...
    use_shared_minecraft_folder: Option<bool>,
    clear_selected_pixelplay_pack: Option<bool>,
    pixelplay_information: Option<crate::state::profile_state::pixelplayInformation>,
}
...
pub async fn create_profile(params: CreateProfileParams) -> Result<Uuid, CommandError> {
    ...
    let profile = Profile {
        ...
        mods: Vec::new(),
        selected_pixelplay_pack_id: params.selected_pixelplay_pack_id.clone(),
        disabled_pixelplay_mods_detailed: HashSet::new(),
...
pub async fn launch_profile(
    ...
    let profile = match state.profile_manager.get_profile(id).await {
        ...
        Err(_) => {
            ...
            let standard_versions = state.pixelplay_version_manager.get_config().await;
...
async fn try_update_profile(id: Uuid, params: UpdateProfileParams) -> Result<(), CommandError> {
    ...
    // Handle selected_pixelplay_pack_id based on clear_selected_pixelplay_pack and new value
    if params.clear_selected_pixelplay_pack == Some(true) {
        info!("Clearing selected_pixelplay_pack_id for profile {}", id);
        profile.selected_pixelplay_pack_id = None;
    } else if let Some(pack_id) = &params.selected_pixelplay_pack_id {
        info!(
            "Updating selected_pixelplay_pack_id to: {} for profile {}",
            pack_id, id
        );
        profile.selected_pixelplay_pack_id = Some(pack_id.clone());
    } else {
        info!("selected_pixelplay_pack_id not explicitly changed or cleared for profile {}. Current: {:?}", id, profile.selected_pixelplay_pack_id);
        // No change to selected_pixelplay_pack_id if neither clear is true nor a new value is provided
    }
    ...
    // Handle pixelplay_information
    if let Some(pixelplay_info) = params.pixelplay_information {
        info!("Updating pixelplay_information to: {:?}", pixelplay_info);
        profile.pixelplay_information = Some(pixelplay_info);
    } else {
        // This else block handles the case where `pixelplay_information` is explicitly `null` in JSON,
        // which Serde maps to `None` for `Option<pixelplayInformation>`.
        // If you want to distinguish between `null` and `undefined` (field not present),
        // you might need `Option<Option<pixelplayInformation>>` or a custom deserializer.
        // For now, if it's `None` (either not sent or sent as null), we keep the existing value.
        // If you want `null` to clear it, you would do: `profile.pixelplay_information = None;`
        info!(
            "pixelplay_information not provided or explicitly null, keeping existing: {:?}",
            profile.pixelplay_information
        );
    }
...
pub async fn resolve_loader_version(
    ...
    let pixelplay_pack_config = state.pixelplay_pack_manager.get_config().await;
    
    let resolved = ModloaderFactory::resolve_loader_version(
        &profile,
        &minecraft_version,
        Some(&pixelplay_pack_config),
    ).await;
...
#[tauri::command]
pub async fn get_standard_profiles() -> Result<pixelplayVersionsConfig, CommandError> {
    info!("Executing get_standard_profiles command");
    let state = State::get().await?;
    let config = state.pixelplay_version_manager.get_config().await;
    Ok(config)
}
...
// Command to retrieve the list of available pixelplay Modpacks
#[tauri::command]
pub async fn get_pixelplay_packs() -> Result<pixelplayModpacksConfig, CommandError> {
    info!("Received command get_pixelplay_packs");
    let state = State::get().await?;
    let config = state.pixelplay_pack_manager.get_config().await;
    Ok(config)
}

/// Retrieves the pixelplay packs configuration with fully resolved mod lists for each pack.
#[tauri::command]
pub async fn get_pixelplay_packs_resolved() -> Result<pixelplayModpacksConfig, CommandError> {
    info!("Received command get_pixelplay_packs_resolved");
    let state = State::get().await?;
    let manager = &state.pixelplay_pack_manager; // Get a reference
    ...
    let resolved_config = pixelplayModpacksConfig {
...
#[tauri::command]
pub async fn set_pixelplay_mod_status(
    profile_id: Uuid,
    pack_id: String,
    mod_id: String,
    game_version: String,
    ...
    info!(
        "Received command set_pixelplay_mod_status: profile={}, pack={}, mod={}, mc={}, loader={}, disabled={}",
    ...
    state
        .profile_manager
        .set_pixelplay_mod_status(profile_id, pack_id, mod_id, game_version, loader, disabled)
        .await?;
    Ok(())
}
...
pub async fn import_profile_from_file(app_handle: tauri::AppHandle) -> Result<(), CommandError> {
    ...
    let dialog_result = tokio::task::spawn_blocking(move || {
        app_handle
            .dialog()
            .file()
            .add_filter("Modpack Files", &["mrpack", "pixelplaypack", "zip"])
            .set_title("Select Modpack File (.mrpack, .pixelplaypack, or .zip)")
    ...
    if let Some(file_path_obj) = dialog_result {
        ...
        let new_profile_id = match file_extension.as_deref() {
            ...
            Some("pixelplaypack") => {
                log::info!("File extension is .pixelplaypack, proceeding with pixelplaypack processing.");
                crate::integrations::pixelplay_packs::import_pixelplaypack_as_profile(file_path_buf)
                    .await?
            }
            Some("zip") => {
            ...
            _ => {
                ...
                return Err(CommandError::from(AppError::Other(
                    "Invalid file type selected. Please select a .mrpack, .pixelplaypack, or .zip file."
                        .to_string(),
                )));
            }
        };
...
pub async fn import_profile(file_path_str: String) -> Result<Uuid, CommandError> {
    ...
    let new_profile_id = match file_extension.as_deref() {
        ...
        Some("pixelplaypack") => {
            log::info!("File extension is .pixelplaypack, proceeding with pixelplaypack processing.");
            crate::integrations::pixelplay_packs::import_pixelplaypack_as_profile(file_path_buf).await?
        }
        Some("zip") => {
            log::info!("File extension is .zip, proceeding with CurseForge modpack processing.");
            curseforge::import_curseforge_pack_as_profile(file_path_buf, None, None).await?
...
pub async fn copy_profile(params: CopyProfileParams) -> Result<Uuid, CommandError> {
    ...
    let new_profile = Profile {
        ...
        selected_pixelplay_pack_id: source_profile.selected_pixelplay_pack_id.clone(),
        disabled_pixelplay_mods_detailed: source_profile.disabled_pixelplay_mods_detailed.clone(),
        ...
        pixelplay_information: source_profile.pixelplay_information.clone(),
...
/// Exports a profile to a .pixelplaypack file format with a fixed export directory
#[tauri::command]
pub async fn export_profile(
    ...
    // Sanitize the filename and add .pixelplaypack extension
    ...
    let pixelplaypack_filename = format!("{}.pixelplaypack", sanitized_name);

    // Create full export path
    let export_path = exports_dir.join(&pixelplaypack_filename);

    info!("Exporting profile to {}", export_path.display());

    // Perform the export
    let result_path = profile_utils::export_profile_to_pixelplaypack(
        params.profile_id,
...
/// Fetches the latest pixelplay packs configuration from the API and updates the local cache.
#[tauri::command]
pub async fn refresh_pixelplay_packs() -> Result<(), CommandError> {
    info!("Refreshing pixelplay packs via command...");
    let state = State::get().await?;
    let config = state.config_manager.get_config().await;

    match state
        .pixelplay_pack_manager
        .fetch_and_update_config(&"", config.is_experimental)
        .await
    {
        Ok(_) => {
            info!("Successfully refreshed pixelplay packs via command.");
            Ok(())
        }
        Err(e) => {
            error!("Failed to refresh pixelplay packs via command: {}", e);
            Err(CommandError::from(e))
        }
    }
}
...
pub async fn refresh_standard_versions() -> Result<Vec<Profile>, CommandError> {
        ...
        .await
    {
        Ok(_) => {
            ...
            let standard_profiles = state.pixelplay_version_manager.get_config().await.profiles;
