...
use crate::minecraft::api::pixelplay_api::CrashlogDto;
...
pub async fn discord_auth_link(app: AppHandle) -> Result<(), CommandError> {
    ...
    let pixelplay_creds = &selected_account_arc.pixelplay_credentials;
    let token = pixelplay_creds.get_token_for_mode(is_experimental)?;

    let url_string = format!(
        "https://api{}.pixelplay.gg/api/v1/core/oauth/discord?token={}",
    ...
    let window =
        WebviewWindowBuilder::new(&app, "discord-signin", WebviewUrl::External(external_url))
            .title("Discord X pixelplayClient")
            .always_on_top(true)
            .center()
    ...
    while (Utc::now() - start_time) < ChronoDuration::minutes(10) {
        match window.url().map_err(|e| {
            ...
            Ok(current_url) => {
                ...
                if current_url_str
                    .starts_with("https://api.pixelplay.gg/api/v1/core/oauth/discord/complete")
                    || current_url_str.starts_with(
                        "https://api-staging.pixelplay.gg/api/v1/core/oauth/discord/complete",
                    )
                {
...
pub async fn discord_auth_status() -> Result<bool, CommandError> {
    ...
    Ok(pixelplayApi::discord_link_status(&token, &account_id_str, is_experimental).await?)
}
...
pub async fn discord_auth_unlink() -> Result<(), CommandError> {
    ...
    pixelplayApi::unlink_discord(&token, &account_id_str, is_experimental).await?;
    Ok(())
}
...
pub async fn get_mobile_app_token() -> Result<String, CommandError> {
    ...
    Ok(pixelplayApi::get_mcreal_app_token(&token, &account_id_str, is_experimental).await?)
}
...
pub async fn reset_mobile_app_token() -> Result<String, CommandError> {
    ...
    Ok(pixelplayApi::reset_mcreal_app_token(&token, &account_id_str, is_experimental).await?)
}
