            ...
            Ok(profile_instance_path) => {
                ...
                if let Some(ref ct) = content_type {
                    dirs_to_scan = match ct {
                        ...
                        _ => dirs_to_scan, // pixelplayMod or others: scan all
                    };
                }
...
pub struct ToggleContentPayload {
    ...
    file_path: Option<String>,
    enabled: bool,
    pixelplay_mod_identifier: Option<crate::state::profile_state::pixelplayModIdentifier>,
...
pub async fn toggle_content_from_profile(
    ...
    log::info!(
        "Attempting to toggle content state: profile_id={}, sha1_hash={:?}, file_path={:?}, enabled={}, pixelplay_mod_identifier={:?}, content_type={:?}",
        payload.profile_id,
        payload.sha1_hash,
        payload.file_path,
        payload.enabled,
        payload.pixelplay_mod_identifier,
    ...
    if let Some(pixelplay_mod_identifier) = payload.pixelplay_mod_identifier {
        log::info!(
            "Toggling pixelplay Pack item state: profile={}, pack={}, mod={}, disabled={}",
            payload.profile_id,
            pixelplay_mod_identifier.pack_id,
            pixelplay_mod_identifier.mod_id,
            !payload.enabled
        );

        // Clone the fields needed for logging
        let pack_id = pixelplay_mod_identifier.pack_id.clone();
        let mod_id = pixelplay_mod_identifier.mod_id.clone();

        // Call set_pixelplay_mod_status with the appropriate parameters
        match state_manager
            .profile_manager
            .set_pixelplay_mod_status(
                ...
                pixelplay_mod_identifier.pack_id,
                pixelplay_mod_identifier.mod_id,
                pixelplay_mod_identifier.game_version,
                pixelplay_mod_identifier.loader,
            ...
            .await
        {
            Ok(_) => {
                log::info!(
                    "Successfully toggled pixelplay Pack item state for pack_id={}, mod_id={} to enabled={}",
                    pack_id,
                    mod_id,
                    payload.enabled
                );
            ...
            Err(e) => {
                log::error!("Failed to toggle pixelplay Pack item state: {}", e);
                return Err(CommandError::from(e));
            }
        }
    }

    // Continue with SHA1-based content toggling if not a pixelplay Pack item
    let current_sha1_hash = match payload.sha1_hash {
        Some(ref hash) => hash.clone(),
        None => {
            log::warn!("SHA1 hash is required for the current toggle implementation when not toggling a pixelplay Pack item.");
            return Err(CommandError::from(AppError::Other(
                "SHA1 hash is required for this toggle operation when not toggling a pixelplay Pack item.".to_string(),
            )));
        }
    };
    ...
    match payload.content_type {
        ...
        Some(profile_utils::ContentType::pixelplayMod) => {
            log::debug!(
                "Targeted toggle for pixelplayMod with SHA1: {}",
                current_sha1_hash
            );
            // pixelplayMods are handled differently, not by scanning directories
            // We don't need to scan any asset types for pixelplayMods
...
pub async fn install_content_to_profile(
    ...
    match payload.content_type {
        ...
        profile_utils::ContentType::pixelplayMod => {
            log::info!("pixelplayMod installation is not supported via this unified command");
            Err(CommandError::from(AppError::Other(
                "pixelplayMod installation not supported via this command".to_string(),
            )))
        }
...
pub async fn install_local_content_to_profile(
    ...
    match payload.content_type {
        ...
        profile_utils::ContentType::pixelplayMod => {
            log::error!(
                "ContentType::pixelplayMod is not supported for local installation via this command. Profile: {}",
                payload.profile_id
            );
            return Err(CommandError::from(AppError::Other(
                "Local installation of pixelplayMod content type is not supported.".to_string(),
            )));
        }
...
pub async fn switch_content_version(
    ...
    match payload.content_type {
        ...
        profile_utils::ContentType::pixelplayMod => {
            log::error!("Switching version for pixelplayMod is not supported via this command.");
            Err(CommandError::from(AppError::InvalidOperation(
                "pixelplayMod versions are managed by pack configuration.".to_string(),
            )))
        }
    }
}
