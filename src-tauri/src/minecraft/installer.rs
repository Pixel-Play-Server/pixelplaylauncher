...
use crate::minecraft::downloads::pixelplayPackDownloadService;
use crate::minecraft::downloads::{ModDownloadService, pixelplayClientAssetsDownloadService};
...
pub async fn install_minecraft_version(
    ...
    // Load pixelplayPackDefinition if a pack is selected
    let pixelplay_pack = if let Some(pack_id) = &profile.selected_pixelplay_pack_id {
    ...
    if let Err(e) = mc_utils::copy_startup_helper_data(profile, &game_directory, pixelplay_pack.as_ref()).await {
    ...
    // Download pixelplayClient assets if profile has a selected pack
    info!("\nDownloading pixelplayClient assets...");

    let pixelplay_assets_service = pixelplayClientAssetsDownloadService::new()
        .with_concurrent_downloads(launcher_config.concurrent_downloads);

    // Download assets for this profile - progress events are now handled internally
    pixelplay_assets_service
        .download_nrc_assets_for_profile(&profile, credentials.as_ref(), is_experimental_mode)
        .await?;

    info!("pixelplayClient Asset download completed!");
    ...
    if modloader_enum != ModLoader::Vanilla {
        ...
        let config_now: pixelplayModpacksConfig = state.pixelplay_pack_manager.get_config().await;
        ...
        if let Some(version) = resolved_loader.version {
            let reason_str = match resolved_loader.reason {
                crate::minecraft::modloader::LoaderVersionReason::pixelplayPack => "pixelplay pack policy",
    ...
    // --- Fetch pixelplay Config Once if a pack is selected ---
    let loaded_pixelplay_config: Option<pixelplayModpacksConfig> = if let Some(pack_id) =
        &profile.selected_pixelplay_pack_id
    {
        info!(
            "Fetching pixelplay config because pack '{}' is selected. Attempting to refresh first.",
            pack_id
        );
        if let Some(creds) = credentials.as_ref() {
            match creds
                .pixelplay_credentials
                .get_token_for_mode(is_experimental_mode)
            {
                Ok(pixelplay_token_value) => {
                    info!("Attempting to update pixelplay pack configuration using obtained token for pack '{}'...", pack_id);
                    if let Err(update_err) = state
                        .pixelplay_pack_manager
                        .fetch_and_update_config(&pixelplay_token_value, is_experimental_mode)
                        .await
                    {
                        warn!(
                                "Failed to update pixelplay pack '{}' configuration: {}. Will proceed with cached version.",
                                pack_id, update_err
                            );
                    } else {
                        info!(
                            "Successfully updated pixelplay pack '{}' configuration from API.",
                            pack_id
                        );
                    }
                }
                Err(token_err) => {
                    warn!(
                            "Could not obtain pixelplay token for pack '{}' to update configuration: {}. Will proceed with cached version.",
                            pack_id, token_err
                        );
                }
            }
            ...
            error!(
                    "A pixelplay pack ('{}') is selected, but no credentials were provided. Cannot attempt to update pack configuration.",
                    pack_id
                );
        }
        ...
        Some(state.pixelplay_pack_manager.get_config().await)
    } else {
        None
    };
    ...
    // --- Step: Download mods from selected pixelplay Pack (if any) ---
    if let Some(selected_pack_id) = &profile.selected_pixelplay_pack_id {
        // Use the already loaded config
        if let Some(config) = loaded_pixelplay_config.as_ref() {
            let pixelplay_mods_event_id = emit_progress_event(
                &state,
                EventType::DownloadingMods,
                profile.id,
                &format!(
                    "Downloading pixelplay Pack '{}' Mods... (Phase 2)",
                    selected_pack_id
                ),
                0.0,
                None,
            ...
            info!(
                "Downloading mods for selected pixelplay Pack '{}'...",
                selected_pack_id
            );

            let pixelplay_downloader_service =
                pixelplayPackDownloadService::with_concurrency(launcher_config.concurrent_downloads);
            let loader_str = modloader_enum.as_str();

            match pixelplay_downloader_service
                ...
                .await
            {
                Ok(_) => {
                    info!(
                        "pixelplay Pack '{}' mods download completed successfully.",
                        selected_pack_id
                    );
                    emit_progress_event(
                        &state,
                        ...
                        profile.id,
                        &format!(
                            "pixelplay Pack '{}' Mods downloaded successfully! (Phase 2)",
                            selected_pack_id
                        ),
                        1.0,
                        None,
                ...
                Err(e) => {
                    error!(
                        "Failed to download pixelplay Pack '{}' mods: {}",
                        selected_pack_id, e
                    );
                    emit_progress_event(
                        &state,
                        EventType::DownloadingMods,
                        profile.id,
                        &format!("Error downloading pixelplay Pack '{}' mods!", selected_pack_id),
                        1.0,
                        Some(e.to_string()),
                    )
                    .await?;
            ...
            error!(
                "pixelplay config was expected but not loaded for pack ID: {}",
                selected_pack_id
            );
        }
    } else {
        info!(
            "No pixelplay Pack selected for profile '{}', skipping pack download.",
            profile.name
        );
    }
    ...
    let target_mods = crate::minecraft::downloads::mod_resolver::resolve_target_mods(
        profile,
        loaded_pixelplay_config.as_ref(), // Pass the reference directly
