...
impl ModloaderFactory {
    ...
    pub async fn resolve_loader_version(
        ...
        pixelplay_pack_config: Option<&pixelplayModpacksConfig>,
        ...
        // 2. Check for pixelplay pack policy
        if let Some(selected_pack_id) = &profile.selected_pixelplay_pack_id {
            if let Some(config) = pixelplay_pack_config {
                if let Ok(resolved_pack) = config.get_resolved_pack_definition(selected_pack_id) {
                    if let Some(policy) = &resolved_pack.loader_policy {
                        ...
                        let get_ver = |m: &std::collections::HashMap<String, crate::integrations::pixelplay_packs::LoaderSpec>| {
                        ...
                        if let Some(version) = resolved_version {
                            return ResolvedLoaderVersion {
                                ...
                                reason: LoaderVersionReason::pixelplayPack,
                            };
                        }
                    }
                }
