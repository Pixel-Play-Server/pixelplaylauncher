...
const PIXELPLAY_ASSETS_DIR: &str = "pixelplayclient";
const DEFAULT_CONCURRENT_DOWNLOADS: usize = 12;

pub struct pixelplayClientAssetsDownloadService {
    base_path: PathBuf,
    concurrent_downloads: usize,
}

impl pixelplayClientAssetsDownloadService {
    pub fn new() -> Self {
    ...
    /// Downloads pixelplay client assets for a specific profile, processing the main pack
    ...
    pub async fn download_nrc_assets_for_profile(
        ...
        let main_pack_id = match &profile.selected_pixelplay_pack_id {
        ...
        let pixelplay_token = match token_ref {
            Some(token) => token.value.clone(),
            None => {
                warn!("[NRC Assets Download] No valid pixelplay token found for {} mode, skipping asset download",
        ...
        info!("[NRC Assets Download] Getting pixelplay packs config...");
        let pixelplay_packs_config = state.pixelplay_pack_manager.get_config().await;

        info!(
            "[NRC Assets Download] Resolving pack definition for main pack: {}",
            main_pack_id
        );
        let resolved_pack_definition = pixelplay_packs_config
        ...
        let target_base_dir = game_directory.join("pixelplayClient").join("assets");

        self.emit_progress_event(
            &state,
            profile.id,
            &format!(
                "Starting pixelplayClient asset processing for {} groups...",
                total_groups
            ),
            0.01,
            None,
        ...
        self.emit_progress_event(
            &state,
            profile.id,
            "pixelplayClient assets processing completed!",
            1.0,
            None,
        )
        .await?;
    ...
    async fn process_asset_group(
        ...
        let assets =
            match pixelplayApi::pixelplay_assets(asset_id, pixelplay_token, request_uuid, is_experimental)
                .await
            {
                Ok(fetched_assets) => {
                    info!(
    ...
    /// Downloads pixelplay client assets for a specific asset ID (pack or asset group).
    async fn download_nrc_assets(
        &self,
        asset_id: &str,
        assets: &pixelplayAssets,
        is_experimental: bool,
        pixelplay_token: &str,
        ...
        let assets_path = self.base_path.join(PIXELPLAY_ASSETS_DIR).join(asset_id);
        ...
        for (name, asset) in assets_list {
            ...
            let pixelplay_token_clone = pixelplay_token.to_string();
            ...
            downloads.push(async move {
                ...
                request = request.header("Authorization", format!("Bearer {}", pixelplay_token_clone));
                            ...
                            tokio::spawn({
                                ...
                                async move {
                                    ...
                                    if let Err(e) = state.emit_event(EventPayload {
                                        event_id,
                                        event_type: EventType::DownloadingpixelplayClientAssets,
    ...
    async fn copy_assets_to_game_dir(
        ...
        let source_dir = self.base_path.join(PIXELPLAY_ASSETS_DIR).join(asset_id);
    ...
    async fn emit_copy_event(
        ...
        state
            .emit_event(EventPayload {
                event_id,
                event_type: EventType::CopyingpixelplayClientAssets,
                target_id: Some(profile_id),
                message: message.to_string(),
                progress: Some(progress.clamp(0.0, 1.0)),
                error,
