...
use crate::integrations::pixelplay_packs::{self, pixelplayModSourceDefinition, pixelplayModpacksConfig};
use crate::state::profile_state::{
    self, CustomModInfo, ModLoader, ModSource, pixelplayModIdentifier, Profile,
};
...
async fn is_filename_blocked_by_config(filename: &str, enable_flagsmith_blocking: bool) -> bool {
    if !enable_flagsmith_blocking {
        return false; // Skip blocking if no pixelplay pack is selected
    }
...
pub async fn resolve_target_mods(
    profile: &Profile,
    pixelplay_config: Option<&pixelplayModpacksConfig>,
    ...
    // Enable Flagsmith blocking only if a pixelplay pack is selected
    let enable_flagsmith_blocking = profile.selected_pixelplay_pack_id.is_some();
    
    if enable_flagsmith_blocking {
        debug!("Flagsmith mod blocking is enabled (pixelplay pack selected)");
    } else {
        debug!("Flagsmith mod blocking is disabled (no pixelplay pack selected)");
    }

    // --- Helper: Get Canonical Key ---
    fn get_canonical_key(source: &pixelplayModSourceDefinition, mod_id: &str) -> Option<String> {
        match source {
            pixelplayModSourceDefinition::Modrinth { project_id, .. } => {
                Some(format!("modrinth:{}", project_id))
            }
            pixelplayModSourceDefinition::Url { .. } => Some(format!("url:{}", mod_id)),
            pixelplayModSourceDefinition::Maven {
                group_id,
                artifact_id,
                ..
    ...
    if let (Some(ref pack_id), Some(config)) = (&profile.selected_pixelplay_pack_id, pixelplay_config) {
        info!("Resolving mods from selected pixelplay Pack: '{}'", pack_id);
        match config.get_resolved_pack_definition(pack_id) {
                ...
                for mod_entry in &pack_definition.mods {
                    ...
                    match ModLoader::from_str(loader_str) {
                        Ok(loader_enum) => {
                            ...
                            if profile.disabled_pixelplay_mods_detailed.contains(&identifier) {
                                info!(
                    ...
                    if let pixelplayModSourceDefinition::Modrinth { project_id, .. } = &mod_entry.source {
                            ...
                            .and_then(|l| l.get(loader_str))
                        {
                            ...
                            {
                                match pixelplay_packs::get_pixelplay_pack_mod_filename(
                                    effective_source,
                                    target,
                                    &mod_entry.id,
                                ) {
                    ...
                    // Handle URL Mods
                    } else if let pixelplayModSourceDefinition::Url { .. } = &mod_entry.source {
                        if let Some(target) = mod_entry
                            .compatibility
                            .get(minecraft_version)
                            .and_then(|l| l.get(loader_str))
                    ...
                    // Handle Maven Mods
                    } else if let pixelplayModSourceDefinition::Maven {
                        repository_ref,
                        group_id,
                        artifact_id,
                    } = &mod_entry.source
            ...
            Err(e) => {
                warn!(
                    "Could not resolve pixelplay Pack definition for pack ID '{}': {}. Skipping pack mods.",
                    pack_id, e
                );
            }
        }
