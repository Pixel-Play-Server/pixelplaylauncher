...
#[derive(Clone)]
pub struct pixelplayPackDownloadService {
    concurrent_downloads: usize,
}

impl pixelplayPackDownloadService {
    pub fn new() -> Self {
        Self {
    ...
    /// Downloads mods specified in a pixelplay pack definition to the central mod cache.
    ...
    pub async fn download_pack_mods_to_cache(
        &self,
        config: &pixelplayModpacksConfig,
        pack_id: &str,
        minecraft_version: &str,
        loader: &str,
    ) -> Result<()> {
        info!(
            "Checking/Downloading pixelplay Pack mods to cache. Pack ID: '{}', MC: {}, Loader: {} (Concurrency: {})",
        ...
        for mod_entry in &pack_definition.mods {
            ...
            download_futures.push(async move {
                ...
                let filename_result =
                    pixelplay_packs::get_pixelplay_pack_mod_filename(effective_source, &target_clone, &mod_id);
                ...
                match effective_source {
                    pixelplayModSourceDefinition::Modrinth {
        ...
        info!(
            "Executing {} pixelplay pack mod cache tasks for pack '{}'...",
        ...
        if errors.is_empty() {
            info!(
                "pixelplay pack mod cache check/download process completed successfully for pack: '{}'",
                pack_id
            );
            Ok(())
        } else {
            error!(
                "pixelplay pack mod cache check/download process completed with {} errors for pack: '{}'",
                errors.len(), pack_id
            );
            Err(errors.remove(0))
        }
...
// not directly on the pixelplay Pack definition. Syncing would still use
// ModDownloadService after the Profile's mod list has been potentially
// updated based on a selected pixelplay Pack.
