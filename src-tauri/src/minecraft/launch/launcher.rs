...
impl MinecraftLauncher {
    ...
    fn create_loggable_command_string(command: &std::process::Command) -> String {
        ...
        while let Some(arg_os_str) = args_iter.next() {
            ...
            if arg_str.starts_with("-Dpixelplay.token=") {
                parts.push("-Dpixelplay.token=*****".to_string());
    ...
    pub async fn launch(
        ...
        let jvm_args = JvmArguments::new(
            natives_path.clone(),
            "pixelplayclient-launcher".to_string(),
            "3.0.0".to_string(),
            classpath.clone(),
        );
        ...
        // Add pixelplay client specific parameters
        // Only add token if we have credentials AND a pixelplay pack is selected in the profile
        let has_pixelplay_pack = profile.as_ref().and_then(|p| p.selected_pixelplay_pack_id.as_ref()).is_some();

        // Add profile name for ingame display
        if let Some(p) = &profile {
            command.arg(format!("-Dpixelplay.profile.name={}", p.name));
        }

        if let Some(creds) = &self.credentials {
            if has_pixelplay_pack {
                // Get the appropriate pixelplay token based on experimental mode setting
                if let Some(pixelplay_token) = if params.is_experimental_mode {
                    info!("[pixelplay Launcher] Using experimental mode token");
                    creds
                    ...
                    info!("[pixelplay Launcher] Using production mode token");
                    creds
                        .pixelplay_credentials
                        .production
                        .as_ref()
                    ...
                    info!("[pixelplay Launcher] Adding pixelplay token to launch parameters");
                    command.arg(format!("-Dpixelplay.token={}", pixelplay_token));
                } else {
                    info!("[pixelplay Launcher] No pixelplay token available for the selected mode");
                }

                // Add experimental mode parameter
                info!(
                    "[pixelplay Launcher] Setting experimental mode: {}",
                    params.is_experimental_mode
                );
                command.arg(format!(
                    "-Dpixelplay.experimental={}",
                    params.is_experimental_mode
                ));
            } else {
                info!("[pixelplay Launcher] No pixelplay pack selected, skipping pixelplay token and experimental mode parameters");
            }
        } else {
            info!("[pixelplay Launcher] No credentials available, skipping pixelplay parameters");
        }
