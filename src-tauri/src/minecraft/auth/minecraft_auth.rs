...
use crate::minecraft::api::pixelplayApi;

#[derive(Debug, Serialize, Deserialize)]
pub struct pixelplayTokenClaims {
    exp: usize,
    username: String,
}
...
pub struct Credentials {
    ...
    pub pixelplay_credentials: pixelplayCredentials,
    pub active: bool,
}

#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct pixelplayCredentials {
    pub production: Option<pixelplayToken>,
    pub experimental: Option<pixelplayToken>,
}

#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct pixelplayToken {
    pub value: String,
    //TODO habs nichts hinbekommen jetzt erstmal bei jedem restart, pub expires: DateTime<Utc>,
}

impl pixelplayCredentials {
    ...
    /// Gets the appropriate pixelplay token based on the experimental mode setting.
    ///
    /// # Arguments
    ...
    pub fn get_token_for_mode(&self, is_experimental: bool) -> Result<String> {
        ...
        token_option
            ...
            .ok_or_else(|| {
                error!(
                    "No pixelplay token found for {} mode.",
                    if is_experimental {
                        "experimental"
                    } else {
                        "production"
...
impl MinecraftAuthStore {
    ...
    /// Create and persist a simple offline account (no external authentication).
    /// The created account will be set as active.
    pub async fn create_offline_account(&self, username: &str) -> Result<Credentials> {
        if username.trim().is_empty() {
            return Err(AppError::AccountError("Username cannot be empty".to_string()));
        }
        ...
        let creds = Credentials {
            ...
            pixelplay_credentials: pixelplayCredentials {
                production: None,
                experimental: None,
            },
            active: false, // will be activated below
        };

        // Insert into store
        self.update_or_insert(creds.clone()).await?;

        // Set as active account (this will save again)
        self.set_active_account(id).await?;

        // Return the stored credentials (now active)
        Ok(self.get_account_by_id(id).await?.ok_or(AppError::NoCredentialsError)?)
    }

    /// Create and persist a simple offline account using a provided UUID (no external authentication).
    /// The created account will be set as active.
    pub async fn create_offline_account_with_id(&self, id: Uuid, username: &str) -> Result<Credentials> {
        if username.trim().is_empty() {
            return Err(AppError::AccountError("Username cannot be empty".to_string()));
        }
    ...
    pub async fn login_finish(&self, code: &str, flow: MinecraftLoginFlow) -> Result<Credentials> {
        ...
        let credentials = Credentials {
            ...
            pixelplay_credentials: match existing_account {
                Some(account) => account.pixelplay_credentials.clone(),
                None => pixelplayCredentials {
                    production: None,
                    experimental: None,
                },
            },
    ...
    pub(crate) async fn refresh_pixelplay_token_if_necessary(
        &self,
        creds: &Credentials,
        force_update: bool,
        ...
        info!(
            "[Token Refresh] Starting pixelplay token refresh check for user: {}",
            creds.username
        );
        ...
        if !force_update {
            ...
            let token_ref = if experimental_mode {
                &creds.pixelplay_credentials.experimental
            } else {
                &creds.pixelplay_credentials.production
            };
            ...
            if let Some(token) = token_ref {
                ...
                match decode::<pixelplayTokenClaims>(&token.value, &key, &validation) {
                    Ok(data) => {
                        info!(
        ...
        if force_update || maybe_update {
            ...
            hasher.update(b"pixelplay-device-salt");
            hasher.update(&hwid);
            ...
            // Use pixelplayApi for token refresh with proper error handling
            info!("[pixelplay Token] Starting token refresh using pixelplayApi");

            // Use the experimental_mode parameter instead of hardcoded value
            info!(
                "[pixelplay Token] Mode: {}",
            ...
            match pixelplayApi::refresh_pixelplay_token_v3(
                &system_id,
                &creds.username,
                &creds.access_token,
            ...
            {
                Ok(pixelplay_token) => {
                    info!("[pixelplay Token] Successfully refreshed token");
                    let mut copied_credentials = creds.clone();

                    if experimental_mode {
                        info!("[pixelplay Token] Storing token in experimental credentials");
                        copied_credentials.pixelplay_credentials.experimental = Some(pixelplay_token);
                    } else {
                        info!("[pixelplay Token] Storing token in production credentials");
                        copied_credentials.pixelplay_credentials.production = Some(pixelplay_token);
                    }

                    // Update the account in storage
                    info!("[pixelplay Token] Updating account in storage");
                ...
                Err(e) => {
                    info!("[pixelplay Token] Token refresh failed: {:?}", e);
                    info!("[pixelplay Token] Falling back to original credentials");
    ...
    async fn refresh_token(&self, creds: &Credentials) -> Result<Option<Credentials>> {
        ...
        let val = Credentials {
            ...
            refresh_token: oauth_token.value.refresh_token,
            expires: oauth_token.date + Duration::seconds(oauth_token.value.expires_in as i64),
            pixelplay_credentials: creds.clone().pixelplay_credentials,
    ...
    pub async fn update_pixelplay_and_microsoft_token(
        &self,
        creds: &Credentials,
        experimental_mode: bool,
        ...
        if creds.expires <= Utc::now() + Duration::minutes(5) {
            ...
            match res {
                Ok(val) => {
                    return if val.is_some() {
                        ...
                        Ok(Some(
                            self.refresh_pixelplay_token_if_necessary(
                                &val.unwrap().clone(),
                                false,
                                experimental_mode,
                            )
            ...
            info!("[Token Check] Checking pixelplay token status");
            Ok(Some(
                self.refresh_pixelplay_token_if_necessary(&creds.clone(), false, experimental_mode)
                    .await?,
            ))
        }
    }
    ...
    pub async fn get_active_account(&self) -> Result<Option<Credentials>> {
        ...
        if let Some(account) = active_account {
            ...
            let updated_account = self
                .update_pixelplay_and_microsoft_token(&account, is_experimental)
                .await?;
