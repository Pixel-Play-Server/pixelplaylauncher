...
impl WordPressApi {
    ...
    pub fn get_api_base() -> String {
        // NOTE: External WordPress API calls were removed to avoid contacting
        // blog.pixelplay.gg automatically. If you want to enable a remote
        // WordPress backend, change this to the appropriate base URL or
        // read it from configuration/environment.
        String::new()
    }
    ...
    pub async fn get_blog_posts(
        ...
        // If the API base is empty, assume WordPress integration is disabled
        // and return an empty result rather than attempting an external call.
        if base_url.trim().is_empty() {
            info!(
                "[WordPress API] API base is empty - WordPress integration disabled. Returning empty list."
            );
            return Ok(Vec::new());
        }
