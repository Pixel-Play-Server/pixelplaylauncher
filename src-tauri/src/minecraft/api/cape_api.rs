...
impl CapeApi {
    ...
    fn get_api_base(is_experimental: bool) -> String {
        if is_experimental {
            ...
            String::from("https://api-staging.pixelplay.gg/api/v1/cosmetics")
        } else {
            debug!("[Cape API] Using production API endpoint");
            String::from("https://api.pixelplay.gg/api/v1/cosmetics")
        }
    }
    ...
    ///
    /// Parameters:
    /// - pixelplay_token: Authentication token
    ...
    pub async fn get_player_capes(
        ...
        debug!(
            ...
            &pixelplay_token[..std::cmp::min(8, pixelplay_token.len())],
            &pixelplay_token[std::cmp::max(0, pixelplay_token.len().saturating_sub(8))..]
        );
        debug!(
