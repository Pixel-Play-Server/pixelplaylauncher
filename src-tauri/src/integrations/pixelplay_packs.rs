...
/// Represents the overall structure of the pixelplay_modpacks.json file.
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct pixelplayModpacksConfig {
    /// A map where the key is the pack ID (e.g., "pixelplay-prod") and the value is the pack definition.
    pub packs: HashMap<String, pixelplayPackDefinition>,
    /// A map defining Maven repositories used by mods with source type "maven".
    /// Key is a reference name (e.g., "pixelplayproduction"), value is the repository URL.
    #[serde(default)] // Allow missing repositories section if no maven mods are used
    pub repositories: HashMap<String, String>,
}

impl Default for pixelplayModpacksConfig {
    fn default() -> Self {
        Self {
            packs: HashMap::new(),
...
/// Defines a single pixelplay modpack variant (e.g., production, development).
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct pixelplayPackDefinition {
    ...
    #[serde(default)]
    pub mods: Vec<pixelplayModEntryDefinition>,
...
pub struct StartUpHelper {
    /// List of relative paths to copy from pixelplayclient/new/ to the profile directory.
...
/// Defines a single mod entry within a pixelplay pack definition.
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct pixelplayModEntryDefinition {
    ...
    pub source: pixelplayModSourceDefinition,
...
/// Defines the general source of a pixelplay mod.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq)]
#[serde(tag = "type", rename_all = "lowercase")]
pub enum pixelplayModSourceDefinition {
    Modrinth {
...
pub struct CompatibilityTarget {
    ...
    pub source: Option<pixelplayModSourceDefinition>,
}
...
/// Helper function to determine the definitive filename for a mod defined within a pixelplay Pack.
...
pub fn get_pixelplay_pack_mod_filename(
    source: &pixelplayModSourceDefinition,
    ...
    match target.filename {
        ...
        None => {
            ...
            match source {
                pixelplayModSourceDefinition::Modrinth { project_slug, .. } => {
                    Ok(format!("{}-{}.jar", project_slug, target.identifier))
                }
                pixelplayModSourceDefinition::Maven { artifact_id, .. } => {
                    Ok(format!("{}-{}.jar", artifact_id, target.identifier))
                }
                pixelplayModSourceDefinition::Url { .. } => {
...
/// Imports a profile from a .pixelplaypack file.
/// This function reads profile.json, creates a new profile, and extracts overrides concurrently.
pub async fn import_pixelplaypack_as_profile(pack_path: PathBuf) -> Result<Uuid> {
    info!("Starting import process for pixelplaypack: {:?}", pack_path);

    // 1. Open the file and create a reader for profile.json initially
    let profile_json_file = File::open(&pack_path).await.map_err(|e| {
        error!(
            "Failed to open pixelplaypack file for profile.json {:?}: {}",
            pack_path, e
        );
        AppError::Io(e)
    })?;
    ...
    let mut zip_for_profile_json = ZipFileReader::with_tokio(&mut profile_json_buf_reader)
        .await
        .map_err(|e| {
            error!("Failed to read pixelplaypack as ZIP for profile.json: {}", e);
            AppError::Other(format!(
                "Failed to read pixelplaypack zip for profile.json: {}",
                e
            ))
        })?;
    ...
    if let Some(file_name) = pack_path.file_stem().and_then(|s| s.to_str()) {
        info!("Using pixelplaypack filename as profile name: {}", file_name);
    ...
    let final_profile_name = if sanitized_base_name.is_empty() {
        let default_name = format!("imported-pixelplaypack-{}", Utc::now().timestamp_millis());
        warn!(
    ...
    let overrides_file_for_listing = File::open(&pack_path).await.map_err(|e| {
        error!(
            "Failed to open pixelplaypack file for overrides listing {:?}: {}",
            pack_path, e
        );
        AppError::Io(e)
    })?;
    ...
    let mut zip_lister_for_overrides = ZipFileReader::with_tokio(&mut overrides_buf_reader)
        ...
        .map_err(|e| {
        error!(
            "Failed to read pixelplaypack as ZIP for overrides listing: {}",
            e
        );
        AppError::Other(format!(
            "Failed to read pixelplaypack zip for overrides: {}",
            e
        ))
    })?;
    ...
    info!(
        "Found {} entries in pixelplaypack. Preparing concurrent streaming for overrides...",
        num_entries
    );
    ...
    for index in 0..num_entries {
        ...
        // Only process the "overrides/" directory for .pixelplaypack files
        if entry_filename_str.starts_with("overrides/") {
            ...
            let sanitized_relative_path_buf = PathBuf::from(path_after_strip_str)
                ...
                .filter_map(|comp| match comp {
                    ...
                    std::path::Component::ParentDir => {
                        warn!("Parent directory component '..' found and removed in pixelplaypack override path: {}", path_after_strip_str);
                        None 
                    }
            ...
            if sanitized_relative_path_buf.as_os_str().is_empty() {
                warn!(
                    "Skipping empty sanitized relative path for pixelplaypack override entry: {} (original relative: {})",
    ...
    if !extraction_tasks.is_empty() {
        ...
        let results = try_join_all(extraction_tasks).await.map_err(|e| {
            error!(
                "Error joining override extraction tasks for pixelplaypack: {}",
                e
            );
            AppError::Other(format!(
                "pixelplaypack override extraction tasks panicked: {}",
                e
            ))
        })?;
        ...
        info!("Successfully extracted all queued overrides for pixelplaypack.");
    } else {
        info!("No override files found or queued for extraction in pixelplaypack.");
    }
...
/// Handles the opening of a .pixelplaypack file, either on app startup or second instance.
/// It will call the `import_profile` command if a valid file path is found in the arguments.
pub async fn handle_pixelplaypack_file_paths<R: tauri::Runtime>(
    app_handle: &tauri::AppHandle<R>,
    args: Vec<String>, // Changed from Vec<PathBuf>
) {
    let mut pixelplaypack_to_import: Option<PathBuf> = None;
    ...
    for arg_str in args_to_check {
        ...
        if let Some(path) = path_candidate {
            if path.is_file() && path.extension().and_then(|ext| ext.to_str()) == Some("pixelplaypack")
            {
                info!("Found .pixelplaypack file to process: {}", path.display());
                pixelplaypack_to_import = Some(path);
                break; // Handle the first .pixelplaypack file found
            }
        }
    }

    if let Some(file_path_to_import) = pixelplaypack_to_import {
        if let Some(file_path_str) = file_path_to_import.to_str() {
                    ...
                    Err(e) => {
                        ...
                        if let Some(window) = import_app_handle.get_webview_window("main") {
                            let error_message = format!(
                                "Failed to import pixelplaypack ({}): {:?}",
                                file_path_to_import.display(),
                                e
                            );
            ...
            error!(
                "Failed to convert .pixelplaypack path to string: {}",
                file_path_to_import.display()
            );
        }
    } else {
        info!("No .pixelplaypack file found in the provided paths.");
    }
}

impl pixelplayModpacksConfig {
    pub fn resolve_pack_mods(
        &self,
        pack_id: &str,
        visited: &mut HashSet<String>, // To detect circular inheritance
    ) -> Result<Vec<pixelplayModEntryDefinition>> {
        ...
        let mut resolved_mods: HashMap<String, pixelplayModEntryDefinition> = HashMap::new();
        ...
        let final_mod_list: Vec<pixelplayModEntryDefinition> = resolved_mods.into_values().collect();
    ...
    pub fn get_resolved_pack_definition(&self, pack_id: &str) -> Result<pixelplayPackDefinition> {
        let base_definition = self.packs.get(pack_id).ok_or_else(|| {
            error!("Pack ID '{}' not found in configuration.", pack_id);
            AppError::Other(format!("Pack ID '{}' not found", pack_id))
        })?;
...
/// Copies a dummy/test `test_pixelplay_modpacks.json` from the project's source directory
/// (assuming a development environment structure) to the launcher's root directory
/// as `pixelplay_modpacks.json` if it doesn't already exist.
///
...
pub async fn load_dummy_modpacks() -> Result<()> {
    ...
    let target_file = target_dir.join("pixelplay_modpacks.json");
    ...
    let source_path = project_root.join("minecraft-data/nrc/pixelplay_modpacks.json");
    // --- End path resolution ---
