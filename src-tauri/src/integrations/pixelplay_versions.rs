...
const PIXELPLAY_API_BASE_URL: &str = "https://api.pixelplayclient.com/v1";

/// Helper to compute versions file path based on experimental flag
fn pixelplay_versions_path_for(is_experimental: bool) -> PathBuf {
    let filename = if is_experimental {
        "pixelplay_versions_exp.json"
    } else {
        "pixelplay_versions.json"
    };
...
pub struct pixelplayVersionsConfig {
    /// A list of standard profiles
    pub profiles: Vec<Profile>,
}

impl Default for pixelplayVersionsConfig {
    fn default() -> Self {
        Self { profiles: vec![] }
    }
}

/// Loads standard profiles from the local `pixelplay_versions.json` file.
/// Returns an empty config if the file doesn't exist.
pub async fn load_local_standard_profiles() -> Result<pixelplayVersionsConfig> {
    let file_path = if let Ok(state) = State::get().await {
        ...
        LAUNCHER_DIRECTORY.root_dir().join("pixelplay_versions.json")
    };
    ...
    let profiles_config: pixelplayVersionsConfig = serde_json::from_str(&data).map_err(|e| {
        error!(
            "Failed to parse local standard profiles file {:?}: {}",
            file_path, e
        );
        AppError::ParseError(format!("Failed to parse pixelplay_versions.json: {}", e))
    })?;
...
/// Copies a dummy/default `pixelplay_versions.json` from the project's source directory
...
pub async fn load_dummy_versions() -> Result<()> {
    ...
    let target_file = if let Ok(state) = State::get().await {
        ...
        target_dir.join("pixelplay_versions.json")
    };
