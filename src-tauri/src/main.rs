...
use crate::commands::process_command::{
    ...
    set_discord_state, stop_process, open_pixelplay_server,
};
use commands::minecraft_auth_command::{
    begin_login, get_accounts, get_active_account, remove_account, set_active_account, create_offline_account, create_offline_account_with_id,
};
...
use commands::profile_command::{
    ...
    get_pixelplay_packs, get_pixelplay_packs_resolved, get_profile, get_profile_directory_structure,
    ...
    open_profile_latest_log, refresh_pixelplay_packs, refresh_standard_versions, repair_profile,
    resolve_loader_version, search_profiles, set_custom_mod_enabled, set_pixelplay_mod_status,
...
use commands::file_command::{
    delete_file, get_icons_for_archives, get_icons_for_pixelplay_mods, open_file, open_file_directory,
...
async fn main() {
    ...
    info!("Iniciando Pixel Play Launcher...");
    ...
    tauri::Builder::default()
        ...
        .plugin(tauri_plugin_single_instance::init(|app, argv, _cwd| {
            info!("Instancia única: segunda instancia iniciada con args: {:?}", argv);
            ...
            // Handle pixelplay:// deep links passed to a second instance by emitting an event
            let app_handle_clone = app.app_handle().clone();
            let argv_clone = argv.clone();
            tauri::async_runtime::spawn(async move {
                for arg in argv_clone.into_iter() {
                    if arg.starts_with("pixelplay://") {
                        debug!("Deep link recibido en segunda instancia: {}", arg);
                        let _ = app_handle_clone.emit("deep_link", arg.clone());
                    }
                }

                // Also handle any .pixelplaypack file arguments
                pixelplay_packs::handle_pixelplaypack_file_paths(&app_handle_clone, argv).await;
            });
        }))
        ...
        .setup(|app| {
            ...
            let show_item = MenuItem::with_id(app, "show", "Mostrar Pixel Play", true, None::<&str>)?;
            ...
            // --- Handle .pixelplaypack file opening or pixelplay:// URIs on initial startup (all platforms) ---
            // The single-instance plugin does not handle the *very first* launch with arguments,
            // so inspect std::env::args() here and forward any pixelplay:// deep links or file args.
            info!("Comprobando argumentos de inicio para deep links o archivos...");
            let startup_args: Vec<String> = std::env::args().collect();
            if startup_args.len() > 1 {
                let handle_clone = app_handle.clone();
                let startup_clone = startup_args.clone();
                tauri::async_runtime::spawn(async move {
                    // Emit deep_link events for any pixelplay:// URIs
                    for arg in startup_clone.iter() {
                        if arg.starts_with("pixelplay://") {
                            debug!("Deep link detectado en arranque: {}", arg);
                            let _ = handle_clone.emit("deep_link", arg.clone());
                        }
                    }

                    // Handle any .pixelplaypack file arguments
                    pixelplay_packs::handle_pixelplaypack_file_paths(&handle_clone, startup_args).await;
                });
            }
            // --- End .pixelplaypack handling on startup ---
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            set_active_account,
            get_accounts,
            create_offline_account_with_id,
