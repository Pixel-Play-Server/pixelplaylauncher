...
// Default Discord application ID can be overridden by launcher config
const DEFAULT_DISCORD_APP_ID: &str = "1423767021371261038"; // Replace if you have official ID
...
pub struct DiscordManager {
    ...
    enabled: Arc<RwLock<bool>>,
        ...
        // Last time the user performed an active action (in seconds since epoch)
        last_user_activity: Arc<RwLock<Option<i64>>>,
    app_id: Arc<RwLock<String>>,
    large_image: Arc<RwLock<String>>,
    large_text: Arc<RwLock<String>>,
}
...
impl DiscordManager {
    pub async fn new(enabled: bool) -> Result<Self> {
        ...
        let manager = Self {
            ...
            last_user_activity: Arc::new(RwLock::new(initial_timestamp)),
            // default values; will be overridden in on_state_ready via config
            app_id: Arc::new(RwLock::new(DEFAULT_DISCORD_APP_ID.to_string())),
            large_image: Arc::new(RwLock::new("Square512x512Logo".to_string())),
            large_text: Arc::new(RwLock::new("Pixel Play".to_string())),
        };

        // Initialize Discord presence if enabled
        if enabled {
            debug!("Discord Rich Presence initially enabled, connecting...");
            // Load optional overrides from config if available
            if let Ok(state) = crate::state::State::get().await {
                let cfg = state.config_manager.get_config().await;
                if let Some(app_id) = cfg.discord_app_id {
                    let mut w = manager.app_id.write().await;
                    *w = app_id;
                }
                if let Some(large_image) = cfg.discord_large_image {
                    let mut w = manager.large_image.write().await;
                    *w = large_image;
                }
                if let Some(large_text) = cfg.discord_large_text {
                    let mut w = manager.large_text.write().await;
                    *w = large_text;
                }
            }
    ...
    async fn connect(&self) -> Result<()> {
        ...
        if client_lock.is_none() {
            ...
            let app_id = self.app_id.read().await.clone();
    ...
    async fn set_state_internal(&self, state: DiscordState, force: bool) -> Result<()> {
        ...
        // Update last_user_activity/idle timestamps based on the new state
        match &state {
            DiscordState::Idle => {
                // Do not update last_user_activity; idle timestamp will be computed when creating activity
            }
            DiscordState::InGame { .. } | DiscordState::Modal(_) => {
                // Active states: update last activity and clear idle timestamp
                let now = SystemTime::now()
                    .duration_since(UNIX_EPOCH)
                    .map(|d| d.as_secs() as i64)
                    .unwrap_or(0);
                let mut last_lock = self.last_user_activity.write().await;
                *last_lock = Some(now);

                let mut idle_lock = self.idle_start_timestamp.write().await;
                *idle_lock = None;
            }
        }
    ...
    async fn create_activity_for_state(&self, state: &DiscordState) -> activity::Activity {
        let icon = self.large_image.read().await.clone(); // Use configured icon name
        // Convert to &'static str by leaking - acceptable for small number of strings used for lifetime with the IPC client
        ...
        match state {
            DiscordState::Idle => {
                // Determine if we should report Idle since last user activity > 5 minutes
                let last_activity = *self.last_user_activity.read().await;
                let now = SystemTime::now()
                    .duration_since(UNIX_EPOCH)
                    .map(|d| d.as_secs() as i64)
                    .unwrap_or(0);

                // If last_activity exists and is older than 5 minutes, use it as start
                let five_minutes = 5 * 60;
                let start_time = if let Some(last) = last_activity {
                    if now - last >= five_minutes {
                        last
                    } else {
                        // Not yet idle for 5 minutes: use current time as start
                        now
                    }
                } else {
                    // No recorded activity, fallback to idle_start_timestamp or now
                    *self.idle_start_timestamp.read().await
                        .as_ref()
                        .unwrap_or(&now)
                };
