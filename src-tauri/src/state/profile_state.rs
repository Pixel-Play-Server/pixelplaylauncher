...
// New struct to uniquely identify a pixelplay Pack mod within a specific context
#[derive(Serialize, Deserialize, Clone, Debug, Eq, PartialEq, Hash)]
pub struct pixelplayModIdentifier {
    pub pack_id: String,
    pub mod_id: String,
...
pub struct Profile {
    ...
    pub selected_pixelplay_pack_id: Option<String>, // ID of the selected pixelplay Pack (e.g., "pixelplay-prod")
    #[serde(default)] // Keep track of disabled mods per pack/version/loader context
    pub disabled_pixelplay_mods_detailed: HashSet<pixelplayModIdentifier>, // Changed field
    ...
    pub pixelplay_information: Option<pixelplayInformation>,
...
#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct pixelplayInformation {
    #[serde(default)]
...
impl ProfileManager {
    ...
    pub async fn get_profile(&self, id: Uuid) -> Result<Profile> {
        ...
        if let Some(profile) = profiles.get(&id).cloned() {
            ...
            // Access global state to get pixelplayVersionManager
            // This assumes State::get() is available and pixelplayVersionManager has get_profile_by_id
            match crate::state::state_manager::State::get().await {
                Ok(state) => {
                    if let Some(standard_profile) =
                        state.pixelplay_version_manager.get_profile_by_id(id).await
                    {
    ...
    // Set the enabled/disabled status of a specific mod within a pixelplay Pack for a profile's specific context
    pub async fn set_pixelplay_mod_status(
        ...
        if let Some(profile) = profiles.get_mut(&profile_id) {
            let identifier = pixelplayModIdentifier {
                pack_id,
                mod_id: mod_id.clone(),
                game_version,
                loader,
            ...
            let changed;
            if disabled {
                changed = profile.disabled_pixelplay_mods_detailed.insert(identifier);
            } else {
                changed = profile.disabled_pixelplay_mods_detailed.remove(&identifier);
            }
    ...
    pub async fn get_profile_instance_path(&self, profile_id: Uuid) -> Result<PathBuf> {
        ...
        match profiles_map.get(&profile_id) {
            ...
            None => {
                ...
                // Get state to access pixelplay_version_manager
    ...
    /// Helper function to check if a group belongs to pixelplay Client
    fn is_pixelplay_client_group(group_name: &str) -> bool {
        let normalized = group_name.to_lowercase();
        normalized == "nrc" || normalized == "pixelplayclient" || normalized == "pixelplay client"
    }
    ...
    pub fn calculate_group_directory(&self, profile: &Profile) -> Result<PathBuf> {
        if let Some(group) = &profile.group {
            if Self::is_pixelplay_client_group(group) {
                // pixelplay Client groups go to "pixelplayclient/legacy" for MC < 1.13, "pixelplayclient/new" otherwise
                if mc_utils::is_legacy_minecraft_version(&profile.game_version) {
                    Ok(default_profile_path().join("pixelplayclient").join("legacy"))
                } else {
                    Ok(default_profile_path().join("pixelplayclient").join("new"))
                }
            } else {
    ...
    /// Synchronizes standard profiles by creating editable copies for each pixelplay_version
    ...
    pub async fn sync_standard_profiles(&self) -> Result<()> {
        ...
        // Get standard profiles from pixelplay version manager
    ...
    async fn update_copy_with_forced_fields(&self, copy_id: Uuid, standard_profile: &Profile) -> Result<bool> {
        ...
        if let Some(copy) = profiles.get_mut(&copy_id) {
            ...
            // Force update pixelplay pack selection if different
            if copy.selected_pixelplay_pack_id != standard_profile.selected_pixelplay_pack_id {
                info!("Updating pixelplay pack for copy {}: {:?} -> {:?}", copy_id, copy.selected_pixelplay_pack_id, standard_profile.selected_pixelplay_pack_id);
                copy.selected_pixelplay_pack_id = standard_profile.selected_pixelplay_pack_id.clone();
                changed = true;
            }
...
impl PostInitializationHandler for ProfileManager {
    async fn on_state_ready(&self, _app_handle: Arc<tauri::AppHandle>) -> Result<()> {
        ...
        // Sync standard profiles - create editable copies for each pixelplay_version
        ...
        // If no profiles exist after loading and syncing standard profiles, create a sensible default
        {
            let profiles_guard = self.profiles.read().await;
            if profiles_guard.is_empty() {
                drop(profiles_guard);
                info!("ProfileManager: No profiles found - creating default profile using latest Minecraft release");

                // Try to fetch latest manifest; fallback to a generic string if anything fails
                let latest_release = match MinecraftApiService::new().get_version_manifest().await {
                    Ok(manifest) => manifest.latest.release,
                    Err(e) => {
                        warn!("Could not fetch Minecraft version manifest for default profile: {}. Falling back to 'latest'", e);
                        "latest".to_string()
                    }
                };
                ...
                let desired_name = format!("Default - {}", latest_release);

                // Attempt to create a unique segment for the profile directory
                match crate::utils::path_utils::find_unique_profile_segment(&base_profiles_dir, &desired_name).await {
                    Ok(unique_segment) => {
                        let profile = Profile {
                            id: Uuid::new_v4(),
                            name: desired_name.clone(),
                            path: unique_segment,
                            game_version: latest_release.clone(),
                            ...
                            mods: Vec::new(),
                            selected_pixelplay_pack_id: None,
                        ...
                        match self.create_profile(profile).await {
                            Ok(id) => info!("Created default profile with id {}", id),
                            Err(e) => warn!("Failed to create default profile: {}", e),
                        }
                    }
                    Err(e) => warn!("Failed to determine unique profile path for default profile: {}", e),
                }
            }
        }
