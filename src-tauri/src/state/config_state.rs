...
pub struct LauncherConfig {
    ...
    #[serde(default)]
    pub discord_large_image: Option<String>,
    #[serde(default)]
    pub discord_small_image: Option<String>,
    #[serde(default)]
    pub discord_large_text: Option<String>,
    #[serde(default)]
...
impl Default for LauncherConfig {
    fn default() -> Self {
        Self {
            ...
            discord_small_image: None,
            discord_large_text: None,
                ...
                match serde_json::from_str::<serde_json::Value>(&config_data) {
                    Ok(json_value) => {
                        ...
                        if let Some(obj) = json_value.as_object() {
                            ...
                            // Ensure new optional fields preserved if present in json
                            migrated_config.discord_app_id = obj.get("discord_app_id").and_then(|v| v.as_str().map(|s| s.to_string()));
                            migrated_config.discord_large_image = obj.get("discord_large_image").and_then(|v| v.as_str().map(|s| s.to_string()));
                            migrated_config.discord_small_image = obj.get("discord_small_image").and_then(|v| v.as_str().map(|s| s.to_string()));
                            migrated_config.discord_large_text = obj.get("discord_large_text").and_then(|v| v.as_str().map(|s| s.to_string()));
                        }
    ...
    pub async fn set_config(&self, new_config: LauncherConfig) -> Result<()> {
        let should_save = {
            ...
            if current.is_experimental == new_config.is_experimental
                ...
                && current.language == new_config.language
            {
                ...
                if current.language != new_config.language {
                    info!(
                        "Changing language: {} -> {}",
                        current.language, new_config.language
                    );
                ...
                *config = LauncherConfig {
                    ...
                    discord_app_id: new_config.discord_app_id.clone(),
                    discord_large_image: new_config.discord_large_image.clone(),
                    discord_small_image: new_config.discord_small_image.clone(),
                    discord_large_text: new_config.discord_large_text.clone(),
