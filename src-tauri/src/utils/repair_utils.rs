...
pub async fn repair_profile_mods(profile_id: Uuid) -> Result<()> {
    ...
    if mod_cache_dir.exists() {
        ...
        // 3. Delete pixelplay pack mod files from cache
        if let Some(pack_id) = &profile.selected_pixelplay_pack_id {
            info!("Cleaning pixelplay pack mod cache for pack: {}", pack_id);
            
            let pixelplay_config = state.pixelplay_pack_manager.get_config().await;
            match pixelplay_config.get_resolved_pack_definition(pack_id) {
                Ok(resolved_pack) => {
                    for pixelplay_mod in &resolved_pack.mods {
                        // Get the cache path for this pixelplay mod
                        match path_utils::get_pixelplay_mod_cache_path(
                            pixelplay_mod,
                            ...
                            Ok(cache_path) => {
                                if cache_path.exists() {
                                    match fs::remove_file(&cache_path).await {
                                        Ok(_) => {
                                            debug!("Removed pixelplay pack mod cache file: {:?}", cache_path);
                                            cache_files_removed += 1;
                                        }
                                        Err(e) => {
                                            warn!("Failed to remove pixelplay pack mod cache file {:?}: {}", cache_path, e);
                                            cache_errors += 1;
                                        }
                                    }
                                } else {
                                    debug!("pixelplay pack mod cache file does not exist: {:?}", cache_path);
                                }
                            }
                            Err(e) => {
                                warn!("Could not determine cache path for pixelplay pack mod {}: {}", 
                                      pixelplay_mod.display_name.as_deref().unwrap_or(&pixelplay_mod.id), e);
                                cache_errors += 1;
                            }
                        }
                    }
                ...
                Err(e) => {
                    warn!("Failed to get pixelplay pack definition for {}: {}", pack_id, e);
                    cache_errors += 1;
                }
            }
        } else {
            debug!("Profile has no selected pixelplay pack, skipping pixelplay mod cache cleanup");
        }
