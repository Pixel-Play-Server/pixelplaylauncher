...
pub fn mask_sensitive_data(content: &str) -> String {
    lazy_static! {
        // Mask pixelplay client tokens
        static ref pixelplay_TOKEN_REGEX: Regex = Regex::new(r"-Dpixelplay\.token=[^\s]+").unwrap();
    ...
    let mut masked_content = pixelplay_TOKEN_REGEX.replace_all(content, "-Dpixelplay.token=*****").to_string();
