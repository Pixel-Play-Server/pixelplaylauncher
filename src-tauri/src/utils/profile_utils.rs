...
use crate::integrations::pixelplay_packs;
...
async fn get_content_directory(profile: &Profile, content_type: &ContentType) -> Result<PathBuf> {
    match content_type {
        ...
        ContentType::pixelplayMod => {
            // pixelplayMods don't have a physical directory but we return a path for consistency
            let state = State::get().await?;
            let instance_path = state
                .profile_manager
                .calculate_instance_path_for_profile(profile)?;
...
fn content_type_to_string(content_type: &ContentType) -> &'static str {
    match content_type {
        ...
        ContentType::Mod => "Mod",
        ContentType::pixelplayMod => "pixelplay Mod",
    }
}
...
/// Represents details about an item when it comes from a pixelplay Pack
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct pixelplayPackItemDetails {
    pub is_enabled: bool,
    pub pixelplay_mod_identifier: Option<crate::state::profile_state::pixelplayModIdentifier>,
}

#[derive(Serialize, Debug, Default, Clone, Deserialize)]
pub struct ContentInstallStatus {
    pub is_included_in_pixelplay_pack: bool,
    pub is_installed: bool,
    pub is_specific_version_in_pack: bool,
    pub is_enabled: Option<bool>,
    pub found_item_details: Option<FoundItemDetails>,
    pub pixelplay_pack_item_details: Option<pixelplayPackItemDetails>,
}

/// Checks the installation status of a specific Modrinth content item within a profile's context.
///
/// Returns a struct indicating if the content is defined in the selected pixelplay Pack
...
pub async fn check_content_installed(params: CheckContentParams) -> Result<ContentInstallStatus> {
    ...
    // --- pixelplay Pack Check (if applicable) ---
    if let Some(pack_id) = &profile.selected_pixelplay_pack_id {
        debug!(
            "Profile {} has selected pixelplay Pack: {}. Checking pack definition...",
            params.profile_id, pack_id
        );
        let config = state.pixelplay_pack_manager.get_config().await;
        match config.get_resolved_pack_definition(pack_id) {
            Ok(resolved_pack) => {
                for pixelplay_mod in &resolved_pack.mods {
                    let mut is_potential_project_match = false;
                    if let (
                        Some(pid_arg),
                        pixelplay_packs::pixelplayModSourceDefinition::Modrinth {
                            project_id: pixelplay_pid,
                            ..
                        },
                    ) = (&params.project_id, &pixelplay_mod.source)
                    {
                        if pid_arg == pixelplay_pid {
                    ...
                    if is_potential_project_match {
                        if let Some(loader_map) = pixelplay_mod.compatibility.get(target_game_version)
                        {
                            if let Some(target) = loader_map.get(target_loader_str) {
                                status.is_included_in_pixelplay_pack = true;
                                ...
                                // New addition: Add pixelplayPackItemDetails
                                let mod_identifier = pixelplay_mod.id.clone();

                                // Create a proper pixelplayModIdentifier
                                let pixelplay_mod_identifier =
                                    crate::state::profile_state::pixelplayModIdentifier {
                                ...
                                let is_pack_mod_enabled = !profile
                                    .disabled_pixelplay_mods_detailed
                                    .contains(&pixelplay_mod_identifier);

                                status.pixelplay_pack_item_details = Some(pixelplayPackItemDetails {
                                    is_enabled: is_pack_mod_enabled,
                                    pixelplay_mod_identifier: Some(pixelplay_mod_identifier),
                                });
                ...
                if status.is_included_in_pixelplay_pack {
                    debug!("Found content (some version) in pixelplay Pack definition.");
                } else {
                    debug!(
                        "Content not found in the definition of pixelplay Pack '{}' for MC {} / {}",
                        pack_id, target_game_version, target_loader_str
                    );
                }
            }
            Err(e) => {
                warn!("Could not resolve pixelplay Pack definition for pack ID '{}': {}. Skipping pack check.", pack_id, e);
            }
        }
    }
...
/// Exports a profile to a `.pixelplaypack` file
///
/// This creates a zip archive with the .pixelplaypack extension that contains:
...
/// @param output_path: Optional path where the .pixelplaypack file should be saved
/// @param include_files: Optional list of files/directories to include in the overrides folder
/// @return: Result containing the path to the created .pixelplaypack file
pub async fn export_profile_to_pixelplaypack(
    profile_id: Uuid,
    output_path: Option<PathBuf>,
    include_files: Option<Vec<PathBuf>>,
) -> Result<PathBuf> {
    info!("Exporting profile {} to .pixelplaypack", profile_id);
    ...
    info!("Creating .pixelplaypack archive at: {}", output_file.display());
...
fn sanitize_profile_for_export(profile: &Profile) -> Profile {
    ...
    // Change pixelplay CLIENT group to CUSTOM for exports
    if let Some(group) = &export_profile.group {
        if group.eq_ignore_ascii_case("pixelplay CLIENT") {
...
async fn process_mod_requests(
    ...
    // For each request, we need to check both in pixelplay Pack and local installation
    for (request, idx) in requests {
        // Convert to the old params format for reusing pixelplay pack check logic
        ...
        // Check if included in pixelplay Pack
        if let Some(pack_id) = &profile.selected_pixelplay_pack_id {
            let state = State::get().await?;
            let config = state.pixelplay_pack_manager.get_config().await;
            ...
            if let Ok(resolved_pack) = config.get_resolved_pack_definition(pack_id) {
                for pixelplay_mod in &resolved_pack.mods {
                    ...
                    ) = (&request.project_id, &pixelplay_mod.source)
                    {
                        if pid_arg == pixelplay_pid {
                            is_potential_project_match = true;
                        }
...
async fn process_resourcepack_requests(
    ...
    for (request, idx) in requests {
        ...
        // Check pixelplay Pack - reuse old function for now
        let _old_params = CheckContentParams {
            profile_id: profile.id,
            project_id: request.project_id.clone(),
            version_id: request.version_id.clone(),
        ...
        if let Some(pack_id) = &profile.selected_pixelplay_pack_id {
            ...
            if let Ok(_resolved_pack) = config.get_resolved_pack_definition(pack_id) {
                ...
                // (Note: This would need to be expanded if pixelplay Packs can contain resource packs)
...
async fn process_shaderpack_requests(
    ...
    for (request, idx) in requests {
        ...
        // Check if in pixelplay Pack - placeholder for future pixelplay Pack shader support
        if let Some(_pack_id) = &profile.selected_pixelplay_pack_id {
...
async fn process_datapack_requests(
    ...
    for (request, idx) in requests {
        ...
        // Check if in pixelplay Pack - placeholder for future pixelplay Pack datapack support
        if let Some(_pack_id) = &profile.selected_pixelplay_pack_id {
            // Placeholder for future implementation
        }
...
pub struct LocalContentItem {
    ...
    pub pixelplay_info: Option<crate::state::profile_state::pixelplayModIdentifier>, // Identifier für pixelplayMods
...
impl LocalContentLoader {
    ...
    pub async fn load_items(
        ...
        let content_dirs = match params.content_type {
            ...
            ContentType::pixelplayMod => {
                // For pixelplay mods, handled differently (no physical directory scan)
                Vec::new()
            }
        };
        ...
        if params.content_type == ContentType::pixelplayMod {
            // Special handling for pixelplay mods - fetch them from the pixelplay pack system
            if let Some(pack_id) = &profile.selected_pixelplay_pack_id {
                // Get the pixelplay pack manager from the state
                let state = State::get().await?;
                let config = state.pixelplay_pack_manager.get_config().await;
                ...
                match config.get_resolved_pack_definition(pack_id) {
                    Ok(pack_def) => {
                        for pixelplay_mod in &pack_def.mods {
                            // Extract fallback version from compatibility target at the beginning
                            let fallback_version = pixelplay_mod
                                .compatibility
                            ...
                            // Create a proper pixelplayModIdentifier first so we can reuse it
                            let pixelplay_mod_identifier =
                                crate::state::profile_state::pixelplayModIdentifier {
                                    pack_id: pack_id.clone(),
                                    mod_id: pixelplay_mod.id.clone(),
                            ...
                            let is_disabled = profile
                                .disabled_pixelplay_mods_detailed
                                .iter()
                                .any(|disabled_mod| *disabled_mod == pixelplay_mod_identifier);

                            // Determine source type string
                            let source_type_str = match &pixelplay_mod.source {
                                crate::integrations::pixelplay_packs::pixelplayModSourceDefinition::Modrinth { .. } => None,
                                crate::integrations::pixelplay_packs::pixelplayModSourceDefinition::Maven { .. } => Some("maven"),
                                crate::integrations::pixelplay_packs::pixelplayModSourceDefinition::Url { .. } => Some("url"),
                            };

                            // Extract Modrinth info if available
                            let modrinth_info = if let crate::integrations::pixelplay_packs::pixelplayModSourceDefinition::Modrinth { project_id, .. } = &pixelplay_mod.source {
                                // For version info we need to look at compatibility
                                let version_id = pixelplay_mod.compatibility
                                    .get(&profile.game_version)
                                ...
                                Some(GenericModrinthInfo {
                                    ...
                                    version_id,
                                    name: pixelplay_mod.display_name.clone().unwrap_or_else(|| pixelplay_mod.id.clone()),
                            ...
                            let path_str = match crate::utils::path_utils::get_pixelplay_mod_cache_path(
                                pixelplay_mod,
                                &profile.game_version,
                                &profile.loader.as_str(),
                            ) {
                                ...
                                Err(e) => {
                                    warn!(
                                        "Could not get cache path for pixelplay mod {}: {}",
                                        pixelplay_mod.id, e
                                    );
                            ...
                            preliminary_items.push(LocalContentItem {
                                filename: pixelplay_mod.id.clone(),
                                path_str,
                                sha1_hash: None,
                                file_size: 0,
                                is_disabled,
                                is_directory: false,
                                content_type: ContentType::pixelplayMod,
                                modrinth_info,
                                curseforge_info: None,
                                platform: None,
                                source_type: source_type_str.map(|s| s.to_string()),
                                pixelplay_info: Some(pixelplay_mod_identifier),
                                fallback_version: fallback_version,
                                id: None,
                                associated_loader: None,
                                modpack_origin: None, // pixelplay mods kommen nicht aus ModPacks
                                updates_enabled: None, // Default behavior
                            });
                        }
                    }
                    Err(e) => {
                        warn!("Failed to get pixelplay pack definition: {}", e);
                    }
                }
            }
        ...
        for content_dir in &content_dirs {
                ...
                entries.next_entry().await.map_err(|e| AppError::Io(e))?
            {
                ...
                let is_valid_item = match params.content_type {
                    ...
                    ContentType::pixelplayMod => false, // We handle pixelplay mods differently, not by scanning directories
                };
        ...
        // If the content type is pixelplayMod, sort the items by filename for consistent ordering
        if params.content_type == ContentType::pixelplayMod {
            final_items.sort_by(|a, b| a.filename.cmp(&b.filename));
        }

        if params.calculate_hashes {
            let hash_tasks: Vec<JoinHandle<(usize, std::result::Result<String, AppError>)>> =
                Vec::new();
        ...
        for (idx, item) in final_items.iter().enumerate() {
            info!(
                "Final item [{}]: filename='{}', path_str='{}', sha1_hash={:?}, file_size={}, is_disabled={}, is_directory={}, content_type={:?}, source_type={:?}, pixelplay_info={:?}, id={:?}, associated_loader={:?}, fallback_version={:?}, modrinth_info={:?}",
                idx,
                item.filename,
                item.path_str,
                item.sha1_hash,
