...
use std::env;
use std::io::Cursor; // Needed for reading NBT from bytes and decompression
...
/// Copies StartUpHelper data from the pixelplayclient/new directory to a new profile's directory.
...
pub async fn copy_startup_helper_data(
    ...
    profile_dir: &PathBuf,
    pixelplay_pack: Option<&crate::integrations::pixelplay_packs::pixelplayPackDefinition>,
) -> Result<()> {
    ...
    if let Err(e) = copy_startup_helper_files(profile, profile_dir, pixelplay_pack).await {
...
/// Copies additional files specified in StartUpHelper from pixelplayclient/new/ directory
...
pub async fn copy_startup_helper_files(
    ...
    // Check if StartUpHelper is configured in pixelplayPack
    let startup_helper = match pixelplay_pack {
        ...
        None => {
            debug!("[{}] No pixelplayPack selected, skipping StartUpHelper.", profile_id);
            return Ok(());
        }
    };
    ...
    // Get the pixelplayclient/new directory path
    let default_profile_path = crate::state::profile_state::default_profile_path();
    let pixelplay_dir = default_profile_path
        .join("pixelplayclient")
        .join("new");

    if !pixelplay_dir.exists() {
        info!(
            "[{}] pixelplayClient new directory not found at: {}, skipping StartUpHelper.",
            profile_id,
            pixelplay_dir.display()
        );
        return Ok(());
    }

    info!(
        "[{}] Found pixelplayClient new directory at: {}",
        profile_id,
        pixelplay_dir.display()
    );
    ...
    for relative_path in &startup_helper.additional_paths {
        let src_path = pixelplay_dir.join(relative_path);
