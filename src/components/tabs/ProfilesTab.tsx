...
export function ProfilesTab() {
  ...
  useEffect(() => {
    ...
    if (currentRouteProfileId && profiles.length > 0 && !loading) {
      ...
      if (profileFromRoute) {
        ...
        toast.error(t("errors.profileNotFound", { id: currentRouteProfileId }));
  ...
  const groupedProfiles = (() => {
    // Separate pixelplay Client profiles and all other profiles
    const pixelplayClientProfiles = initiallyFilteredProfiles.filter(profile => 
      profile.group === "pixelplay CLIENT"
    );
    ...
    // Sort both groups
    pixelplayClientProfiles.sort((a, b) => a.name.localeCompare(b.name));
    otherProfiles.sort((a, b) => a.name.localeCompare(b.name));
    
    const result: Record<string, Profile[]> = {};
    
    // Always add pixelplay Client group first if there are profiles with that group
    if (pixelplayClientProfiles.length > 0) {
      result["pixelplay Client"] = pixelplayClientProfiles;
    }
  ...
  const sortedGroupKeys = Object.keys(groupedProfiles).sort((a, b) => {
    // pixelplay Client should always be first
    if (a === "pixelplay Client") return -1;
    if (b === "pixelplay Client") return 1;
  ...
  const handleDeleteProfile = async (
    ...
    toast.promise(deletePromise, {
      loading: t("profiles.deleteProfile") + ` '${profileName}'...`,
      success: () => {
        fetchProfiles();
        ...
        return t("profiles.profileDeleted");
      },
      error: (err) =>
        t("errors.failedToDeleteProfile", { error: err instanceof Error ? err.message : String(err.message) }),
    });
  };
  ...
  const handleGroupingChange = async (newCriterion: string) => {
    ...
    try {
      ...
      toast.error(t("errors.failedToSaveGrouping"));
    }
  };
  ...
  const groupingOptions = [
    ...
    {
      value: "game_version",
      label: t("profiles.grouping.gameVersion"),
