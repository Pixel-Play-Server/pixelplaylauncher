...
export function GeneralStep({
  ...
  const pixelplayPackOptions = Object.entries(pixelplayPacks).map(
    ([packId, packDef]) => ({
      value: packId,
          ...
          {loading ? (
            ...
            <>
              <Select
                ...
                onChange={(value) =>
                  updateProfile({
                    selected_pixelplay_pack_id: value === "" ? null : value,
                  })
                }
                options={[
                  { value: "", label: "None (Optional)" },
                  ...pixelplayPackOptions,
                ]}
              />
              {profile.selected_pixelplay_pack_id &&
                pixelplayPacks[profile.selected_pixelplay_pack_id] && (
                  <Card
                    variant="flat"
                    className="mt-4 p-4 bg-black/20 border border-white/10"
                  >
                    <p className="text-xs text-white/80 font-minecraft-ten tracking-wide">
                      {pixelplayPacks[profile.selected_pixelplay_pack_id].description}
                    </p>
                  </Card>
                )}
            </>
