...
const LOCAL_CONTENT_TAB_ICONS_TO_PRELOAD = [
  ...
  "solar:shield-cross-bold-duotone", // For pixelplay blocked badge
];
...
export function LocalContentTabV2<T extends LocalContentItem>({
  ...
  // Fetch Flagsmith config when a pixelplay pack is selected
  useEffect(() => {
    ...
    if (profile?.selected_pixelplay_pack_id) {
      FlagsmithService.getBlockedModsConfig()
        ...
        .catch((err) => {
          console.error("Failed to load pixelplay blocked mods config:", err);
  ...
  useEffect(() => {
    ...
    if (needsPackConfig && profile) {
      const fetchPacks = async () => {
        ...
        try {
          const config = await ProfileService.getpixelplayPacksResolved();
          setpixelplayPacksConfig(config);
        } catch (err) {
          console.error("Failed to fetch pixelplay packs config:", err);
          toast.error("Failed to load pixelplay pack list.");
      ...
      setpixelplayPacksConfig(null); // Clear if not pixelplayMod or no profile
    }
  }, [contentType, profile]);

  const handleRefreshPacksList = useCallback(async () => {
    if (contentType !== "pixelplayMod") return;
    setIsRefreshingPacksList(true);
    try {
      await ProfileService.refreshpixelplayPacks();
      const config = await ProfileService.getpixelplayPacksResolved();
      setpixelplayPacksConfig(config);
      toast.success("pixelplay Pack list refreshed.");
    } catch (err) {
      console.error("Failed to refresh pixelplay packs list:", err);
      toast.error("Failed to refresh pixelplay pack list.");
    } finally {
  ...
  const pixelplayPackOptions = useMemo((): SelectOption[] => {
    if (contentType !== "pixelplayMod" || !pixelplayPacksConfig) {
  ...
  }, [contentType, pixelplayPacksConfig]);

  const handleSelectedPackChange = useCallback(
    async (newPackId: string | null) => {
      if (!profile || newPackId === profile.selected_pixelplay_pack_id) return;
      try {
  ...
  const renderListItem = useCallback(
    (item: T) => {
      ...
      const isBlockedBypixelplay =
        !!profile?.selected_pixelplay_pack_id &&
        isBlockedConfigLoaded &&
        FlagsmithService.isModBlockedBypixelplay(
          item.filename,
      ...
      const itemBadgesNode = [
        // pixelplay crash warning (highest priority)
        ...(isBlockedBypixelplay ? [{
      ...
      // Note: pixelplay mods can also have updates available, but they won't be auto-updatable
      ...
      if (isCurrentlyUpdating && !item.pixelplay_info) {
        itemActions.push({
          id: "updating",
      ...
      // Delete action (if not pixelplay mod) - icon-only
      if (!item.pixelplay_info) {
        itemActions.push({
          id: "delete",
    ...
    <div className="flex flex-col flex-grow min-w-0">
      ...
      <>
        <div className="flex items-center justify-between w-full min-h-14">
          ...
          {/* Right side: Action Buttons and pixelplayPack Dropdown */}
          <div className="flex items-center gap-2">
            ...
            {selectedItemIds.size === 0 && (
              <>
                {/* pixelplay Pack Selector - Only for pixelplayMod type */}
                {contentType === "pixelplayMod" &&
                  pixelplayPacksConfig &&
                  pixelplayPackOptions.length > 0 && (
                    <div className="flex items-center gap-2">
                      <Select
                ...
                {/* Update All buttons - Only for non-pixelplayMod types */}
                {contentType !== "pixelplayMod" && updatableContentCount > 0 && (
                  <ContentActionButtons
                    actions={[
                      {
                        id: "update-all",
  ...
  const showpixelplayPackSelector = contentType === "pixelplayMod";
  const ispixelplayPackSelected =
    showpixelplayPackSelector && profile?.selected_pixelplay_pack_id;

  // Dynamic empty state messages
  const getEmptyStateMessage = () => {
    if (contentType === "pixelplayMod" && !profile?.selected_pixelplay_pack_id) {
      return "No pixelplay Pack Selected";
    } else if (error) {
  ...
  const getEmptyStateDescription = () => {
    if (contentType === "pixelplayMod" && !profile?.selected_pixelplay_pack_id) {
      return "Please select a pixelplay Modpack from the dropdown to manage its mods.";
    } else if (error) {
  ...
  return (
    <>
      <GenericContentTab<T>
        items={
          contentType === "pixelplayMod" && !profile?.selected_pixelplay_pack_id
            ? []
            : filteredItems
        }
        ...
        emptyStateAction={
          // Show browse button for empty states (except when pixelplay pack not selected and when loading)
          (isTrulyEmptyState ||
