...
import {
  getpixelplayPacks,
  getpixelplayPacksResolved,
  refreshpixelplayPacks,
} from "../../../services/profile-service";
import type { Profile } from "../../../types/profile";
import type { pixelplayModpacksConfig } from "../../../types/pixelplayPacks";
...
interface pixelplayModsTabProps {
  profile: Profile;
  onRefresh?: () => void;
  isActive?: boolean;
  searchQuery?: string;
...
export function pixelplayModsTab({
  profile,
  onRefresh,
  isActive = false,
  searchQuery = "",
}: pixelplayModsTabProps) {
  const [pixelplayMods, setpixelplayMods] = useState<pixelplayMod[]>([]);
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      ...
      try {
        await fetchpixelplayPacks();

        try {
          await refreshpixelplayPacks();
          await fetchpixelplayPacks();
        } catch (refreshError) {}
  ...
  useEffect(() => {
    if (profile.selected_pixelplay_pack_id && pixelplayPacksConfig) {
      fetchpixelplayMods();
    } else {
      setpixelplayMods([]);
      setIsLoading(false);
    }
  }, [profile.id, profile.selected_pixelplay_pack_id, pixelplayPacksConfig]);

  const fetchpixelplayPacks = async () => {
    try {
      setError(null);

      try {
        const result = await getpixelplayPacksResolved();
        setpixelplayPacksConfig(result);
      } catch (resolvedError) {
        const basicResult = await getpixelplayPacks();
        setpixelplayPacksConfig(basicResult);
      }
    } catch (error) {
      setError(
        `Failed to load pixelplay packs: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };

  const fetchpixelplayMods = async () => {
    ...
    try {
      try {
        const modsResult = await invoke<any>("get_pixelplay_pack_mods", {
        ...
        if (!packDef) {
          setError(
            `pixelplay pack "${profile.selected_pixelplay_pack_id}" not found. Try refreshing the packs.`,
          );
          setIsLoading(false);
          return;
        }
        ...
        if (
          ...
          try {
            const lastResortResult = await invoke<any>("list_pixelplay_mods", {
            ...
            if (lastResortResult && Array.isArray(lastResortResult)) {
              ...
              setError(
                "Could not load pixelplay mods. No mods found in pack definition.",
              );
              setpixelplayMods([]);
            }
          } catch (lastResortError) {
            setError(
              `Failed to load pixelplay mods: ${
      ...
      setError(
        `Failed to load pixelplay mods: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const ispixelplayModDisabled = (packModId: string): boolean => {
    if (
  ...
  const processFetchedMods = async (mods: any[]) => {
    try {
      ...
      const modsWithStatus = compatibleMods.map((mod) => {
        const isDisabled = ispixelplayModDisabled(mod.id);
      ...
      setpixelplayMods(modsWithStatus);
  ...
  const handleToggleMod = async (modId: string) => {
    if (!profile.selected_pixelplay_pack_id) return;
  ...
  const handleRefresh = async () => {
    ...
    try {
      await refreshpixelplayPacks();
      await fetchpixelplayPacks();
      await fetchpixelplayMods();
      if (onRefresh) onRefresh();
    } catch (error) {
      setError(
        `Failed to refresh pixelplay packs: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      setRefreshing(false);
    }
  ...
  const filteredMods = pixelplayMods.filter(
    (mod) =>
      mod.display_name
        .toLowerCase()
  ...
  const currentPackName = profile.selected_pixelplay_pack_id
    ? pixelplayPacksConfig?.packs[profile.selected_pixelplay_pack_id]?.displayName ||
      "Unknown Pack"
    : "No Pack Selected";

  const isExperimental = profile.selected_pixelplay_pack_id
    ? pixelplayPacksConfig?.packs[profile.selected_pixelplay_pack_id]
          ...
          <div className="h-full flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-white/40 font-minecraft text-sm mt-2 tracking-wide lowercase select-none">
                select a pixelplay pack in profile settings
              </p>
            </div>
          </div>
        ) : isLoading ? (
          <LoadingState message="loading pixelplay mods..." />
        ) : error ? (
              ...
              <EmptyState
                ...
                message={
                  effectiveSearchQuery
                    ...
                    : "no pixelplay mods available"
                }
                description="pixelplay mods are automatically managed by the launcher"
              />
            )}
          </ContentTable>
        )}
...
interface pixelplayModRowProps {
  mod: pixelplayMod;
  isSelected: boolean;
  onSelect: () => void;
  onToggle: () => void;
...
function PixelplayModRow({
  mod,
  isSelected,
  onSelect,
  onToggle,
  localIcon,
}: pixelplayModRowProps) {
