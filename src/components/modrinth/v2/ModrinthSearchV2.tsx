...
export function ModrinthSearchV2({
  ...
  const checkDisplayedVersionsStatus = async (projectId: string, versions: UnifiedVersion[], startIndex: number, count: number, forceRefresh: string[] = []) => {
    ...
    try {
      ...
      // First request for just the project to check pixelplay pack status
      requests.push({
        project_id: projectId,
      ...
      let projectInpixelplayStatus: ContentInstallStatus | null = null;
      
      batchResults.results.forEach(result => {
        if (result.request_id === `project-${projectId}`) {
          // This is the project-level check for pixelplay pack
          projectInpixelplayStatus = result.status;
        } else if (result.request_id) {
          // This is a version check
          newInstalledState[result.request_id] = {
            ...result.status,
            // If project is in pixelplay pack, set is_included_in_pixelplay_pack based on version match
            is_included_in_pixelplay_pack: 
              projectInpixelplayStatus?.is_included_in_pixelplay_pack && result.status.is_specific_version_in_pack
          };
        }
      });
      ...
      try {
        const projectInpixelplayStatus = await ProfileService.isContentInstalled({
          profile_id: selectedProfile.id,
          project_id: projectId,
          project_type: projectType
        });
          ...
          try {
            ...
            if (!primaryFile) {
              newInstalledState[version.id] = {
                is_installed: false,
                is_included_in_pixelplay_pack: false,
                is_specific_version_in_pack: false,
                is_enabled: null,
                found_item_details: null,
                pixelplay_pack_item_details: null,
              };
              continue;
            }
            ...
            newInstalledState[version.id] = {
              ...
              is_included_in_pixelplay_pack: projectInpixelplayStatus.is_included_in_pixelplay_pack && statusFromService.is_specific_version_in_pack,
              ...
              pixelplay_pack_item_details: statusFromService.pixelplay_pack_item_details || null,
            };
          } catch (error) {
        ...
        success: (data: any) => {
          ...
          setInstalledVersions(prev => {
            ...
            newState[profileId][version.id] = { // Use profileId
              is_installed: false,
              is_included_in_pixelplay_pack: newState[profileId]?.[version.id]?.is_included_in_pixelplay_pack || false, // Use profileId
              is_specific_version_in_pack: newState[profileId]?.[version.id]?.is_specific_version_in_pack || false, // Use profileId
              is_enabled: null,
              found_item_details: null,
              pixelplay_pack_item_details: newState[profileId]?.[version.id]?.pixelplay_pack_item_details || null, // Use profileId
            };
          ...
          if (!anyVersionsStillInstalled && selectedProfile && selectedProfile.id === profileId) {
            setInstalledProjects(prev => ({
              ...
              [project.project_id]: {
                is_installed: false,
                is_included_in_pixelplay_pack: prev[project.project_id]?.is_included_in_pixelplay_pack || false,
                is_specific_version_in_pack: prev[project.project_id]?.is_specific_version_in_pack || false,
                is_enabled: null,
                found_item_details: null,
                pixelplay_pack_item_details: prev[project.project_id]?.pixelplay_pack_item_details || null,
              }
            }));
          }
  ...
  const handleToggleEnableVersion = async (
    ...
    // Check if this is a pixelplay Pack item
    if (currentVersionStatus?.pixelplay_pack_item_details?.pixelplay_mod_identifier) {
      const pixelplayIdentifier = currentVersionStatus.pixelplay_pack_item_details.pixelplay_mod_identifier;
      ...
      await toast.promise(
        async () => {
          const payload: ToggleContentPayload = {
            ...
            pixelplay_mod_identifier: pixelplayIdentifier,
            content_type: nrContentType, // Pass content_type here as well
            // sha1_hash is not strictly needed for pixelplay_mod_identifier-based toggling by current backend logic,
          ...
          setInstalledVersions(prev => {
            ...
            if (newState[selectedProfile.id][version.id]) {
              newState[selectedProfile.id][version.id] = {
                ...
                pixelplay_pack_item_details: {
                  ...newState[selectedProfile.id][version.id]!.pixelplay_pack_item_details!,
                  is_enabled: newEnabledState
                }
              };
            }
              ...
              return {
                ...
                [project.project_id]: {
                  ...
                  pixelplay_pack_item_details: {
                    ...
                    ...(currentProjectStatus.pixelplay_pack_item_details || {}),
                    // We might not have a full pixelplay_mod_identifier here at project level,
        ...
        {
          loading: `${toastMessage} pixelplay Pack item: ${project.title} (${version.version_number})...`,
          success: ({ versionName }) => `Successfully ${successMessage} pixelplay Pack item: ${project.title} (${versionName})`,
          error: (err) => `Failed to ${toastMessage.toLowerCase()} pixelplay Pack item: ${err.message || String(err)}`
        }
      ).catch(err => {
        console.error(`Error ${toastMessage.toLowerCase()} pixelplay Pack item:`, err);
      });
      
      return; // Exit after handling pixelplay pack item
    }

    // Regular content toggle using SHA1 hash (for non-pixelplay pack items)
    if (!sha1Hash) {
    ...
    await toast.promise(
      async () => {
        const payload: ToggleContentPayload = {
          ...
          pixelplay_mod_identifier: undefined, // Explicitly undefined for non-pixelplay items
        };
  ...
  const getStatusForNewInstall = (
    ...
    is_installed: true,
    is_included_in_pixelplay_pack: existingPreviousStatus?.is_included_in_pixelplay_pack || false,
    ...
    pixelplay_pack_item_details: existingPreviousStatus?.pixelplay_pack_item_details || null,
  });
