...
export const ModrinthVersionItemV2 = React.memo<ModrinthVersionItemV2Props>(
  ({
    ...
    const handleToggleEnableButtonClick = () => {
      if (versionStatus?.pixelplay_pack_item_details?.pixelplay_mod_identifier) {
    ...
    if (project.project_type === "modpack" && isInstallingModpackVersion) {
      ...
      if (versionStatus?.is_included_in_pixelplay_pack && !isModpack) {
        buttonText = "In Pack";
        buttonVariant = "secondary";
        buttonDisabled = true;
    ...
    const showInstallBorder =
      selectedProfileId &&
      (versionStatus?.is_installed ||
        versionStatus?.is_included_in_pixelplay_pack);
              ...
              <div className="flex gap-1 flex-shrink-0">
                ...
                {selectedProfileId &&
                  ((versionStatus?.is_installed &&
                    ...
                    onToggleEnableClick) ||
                    (versionStatus?.is_included_in_pixelplay_pack &&
                      versionStatus?.pixelplay_pack_item_details &&
                      onToggleEnableClick)) && (
                    <ActionButton
