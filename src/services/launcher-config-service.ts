...
/**
 * Sets the language preference in the launcher configuration.
 * Fetches the current config, updates the language, and saves it back.
 * @param language The new language code (e.g., "en", "es").
 * @returns A promise that resolves when the language is successfully set.
 * @throws If fetching or setting the config fails.
 */
export async function setLanguagePreference(language: string): Promise<void> {
  console.log(`[LauncherConfigService] Setting language preference to: ${language}`);
  try {
    const currentConfig = await getLauncherConfig();
    const newConfig: LauncherConfig = {
      ...currentConfig,
    ...
    await setLauncherConfig(newConfig);
    console.log("[LauncherConfigService] Successfully set language preference.");
  } catch (error) {
    console.error("[LauncherConfigService] Failed to set language preference:", error);
    throw error;
  }
} 