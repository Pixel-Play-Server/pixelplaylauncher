...
/**
 * Triggers a refresh of the pixelplay packs configuration from the backend.
 *
 * @returns A promise that resolves when the refresh is complete.
 * @throws If the backend command fails.
 */
export const refreshpixelplayPacks = (): Promise<void> => {
  return invoke('refresh_pixelplay_packs');
};
...
/**
 * Refreshes both pixelplay packs and standard versions configurations.
...
export const refreshNrcDataOnMount = async (): Promise<void> => {
  ...
  try {
    ...
    try {
      ...
      console.log("pixelplay Packs updated successfully on mount!");
      nrcPacksSuccess = true;
    } catch (error) {
      console.error("Failed to refresh pixelplay Packs on mount:", error);
    }
    ...
    // Dieser Catch fängt Fehler von refreshpixelplayPacks, refreshStandardVersions
...
/**
 * Gets the mobile app token for pixelplay mobile app linking.
 *
...
/**
 * Resets the mobile app token for pixelplay mobile app linking.
 *
