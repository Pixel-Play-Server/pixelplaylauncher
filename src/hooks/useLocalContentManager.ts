...
export type LocalContentType = 'ShaderPack' | 'ResourcePack' | 'DataPack' | 'Mod' | 'pixelplayMod';
...
function mapUiContentTypeToBackend(uiType: LocalContentType): NrContentType {
  switch (uiType) {
    ...
    case 'pixelplayMod': return NrContentType.pixelplayMod;
...
function mapBackendItemToFrontendType<T extends LocalContentItem>(rawItem: ProfileLocalContentItem): T {
  ...
  // It has fields like: filename, path_str, ..., pixelplay_identifier, fallback_version
  // The actual object from Rust via invoke might have `pixelplay_info` field instead of `pixelplay_identifier` being populated.
  const outputItem = {
    ...rawItem, // Spread all properties from rawItem (which is typed as ProfileLocalContentItem)
    path: rawItem.path_str, // Add/override path using path_str from ProfileLocalContentItem // Fallback to the typed pixelplay_identifier if pixelplay_info isn't there
  };
...
function createUninstallPayload<T extends LocalContentItem>(
  ...
  if (uiContentType === 'Mod') {
    ...
    return { profile_id: profileId, file_path: item.path };
  } else if (uiContentType === 'pixelplayMod') {
    toast.error("Direct uninstallation of pixelplayMod items is not supported via this method. Please manage pixelplay Packs directly.");
    console.error("[useLocalContentManager] Attempted to create uninstall payload for pixelplayMod. This is generally not supported here.");
    return null;
  }
...
function createTogglePayload<T extends LocalContentItem>(
  ...
  const payloadBase: Omit<ToggleContentPayload, 'sha1_hash' | 'file_path' | 'pixelplay_mod_identifier'> = {
    profile_id: profileId,
    enabled: targetEnabledState,
    content_type: backendContentType,
  };

  if (uiContentType === 'pixelplayMod') {
    const pixelplayIdentifierFromItem = (item as ProfileLocalContentItem).pixelplay_info; // Expect pixelplay_info from the item
    if (pixelplayIdentifierFromItem) {
      return { ...payloadBase, pixelplay_mod_identifier: pixelplayIdentifierFromItem }; // Map to payload's pixelplay_mod_identifier
    } else {
      toast.error(`pixelplayMod item ${item.filename} is missing the pixelplay_info. Cannot toggle.`);
      return null;
    }
...
export function useLocalContentManager<T extends LocalContentItem>({
  ...
  }, [fetchBasicInfo, profile?.selected_pixelplay_pack_id]); // Added profile.selected_pixelplay_pack_id to ensure refetch on pack change
    ...
    const fetchLocalArchiveIcons = async () => {
      ...
      const pathsToFetchIconsFor = items
        .filter(item => {
          ...
          // For pixelplayMod, the item.path points to a .jar file in cache
          // For other types, item.path usually points to a .zip file
          const lowerPath = item.path.toLowerCase();
          if (contentType === 'pixelplayMod') {
  ...
  const handleToggleItemEnabled = useCallback(async (item: T) => {
    ...
    try {
      ...
      if (contentType !== 'pixelplayMod' && onRefreshRequiredRef.current) {
  ...
  const checkForContentUpdates = useCallback(async (currentProfile = profile, currentItems = items) => {
    ...
    try {
      const request: UnifiedUpdateCheckRequest = {
        hashes,
        algorithm: "sha1",
        loaders: (contentType === 'Mod' || contentType === 'pixelplayMod') && currentProfile.loader ? [currentProfile.loader] : [],
  ...
  const handleUpdateContentItem = useCallback(async (item: T, updateVersion: UnifiedVersion, suppressOwnToast: boolean = false) => {
    ...
    if (!item.path && !(contentType === 'Mod' && item.id && !item.source_type && !item.pixelplay_info)) {
      toast.error(`Item path missing for ${getDisplayFileName(item)}, cannot update.`);
      return;
    }

    // Platform-specific validation
    if (platform === 'Modrinth' && contentType === 'Mod' && item.id && !item.source_type && !item.pixelplay_info && !item.modrinth_info) {
      toast.error(`Mod ${getDisplayFileName(item)} is not recognized as a Modrinth mod. Cannot update.`);
      return;
    }

    if (platform === 'CurseForge' && contentType === 'Mod' && item.id && !item.source_type && !item.pixelplay_info && !item.curseforge_info) {
