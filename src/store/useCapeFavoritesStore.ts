...
interface CapeFavoritesState {
  ...
  clearFavorites: () => void;
  setFavoriteOptimistic: (capeId: string, favorite: boolean, pixelplayToken?: string) => Promise<void>;
  toggleFavoriteOptimistic: (capeId: string, pixelplayToken?: string) => Promise<void>;
}

const STORAGE_KEY = "pixelplay-cape-favorites";
...
export const useCapeFavoritesStore = create<CapeFavoritesState>()(
  persist(
    (set, get) => ({
      ...
      setFavoriteOptimistic: async (capeId: string, favorite: boolean, pixelplayToken?: string) => {
        ...
        try {
          ...
          await setCapeFavorite(capeId, favorite, pixelplayToken);
        } catch (err) {
      ...
      toggleFavoriteOptimistic: async (capeId: string, pixelplayToken?: string) => {
        const nextFavorite = !get().isFavorite(capeId);
        await get().setFavoriteOptimistic(capeId, nextFavorite, pixelplayToken);
      },
    }),
    {
      name: STORAGE_KEY,
