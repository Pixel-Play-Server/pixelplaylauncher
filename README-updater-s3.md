...
- Subiremos los artifacts al bucket bajo una estructura tipo: `releases-v2/<channel>/<arch>/<version>/`.
- Expondremos el bucket mediante un Cloudflare Worker (o un dominio + CDN) para servir por HTTPS.
- El launcher buscará manifests en `BASE_URL/{{target}}/{{arch}}/{{current_version}}`.
...
4) Configura el launcher para apuntar al bucket/Worker:
   - En CI (recomendado) define `PIXELPLAY_UPDATER_BASE_URL_COMPILED='https://mi-cdn.example.com/releases-v2'` antes del build para incrustarlo en el binario.
   - Para pruebas locales define `PIXELPLAY_UPDATER_BASE_URL='https://mi-cdn.example.com/releases-v2'` en el entorno y ejecuta `yarn dev`.

5) Estructura esperada en el bucket (ejemplo):
   - `releases-v2/stable/x86_64/0.6.10/manifest.json`
   - `releases-v2/stable/x86_64/0.6.10/pixelplay-Windows-setup.exe`
   - `releases-v2/stable/x86_64/0.6.10/pixelplay-Windows-setup.exe.sig`

Comandos útiles locales
 - Subir artifacts con AWS CLI (usar endpoint R2):
```bash
aws --endpoint-url "https://<account_id>.r2.cloudflarestorage.com" s3 sync ./artifacts s3://my-bucket/releases-v2/ --exact-timestamps
```

 - Subir con el script incluido:
```bash
chmod +x scripts/upload_to_r2.sh
./scripts/upload_to_r2.sh ./artifacts my-bucket https://<account_id>.r2.cloudflarestorage.com releases-v2 stable
```
...
Problemas comunes
- Si el updater no ve la nueva versión, revisa que la URL construida por el launcher coincida con la ruta en el bucket.
